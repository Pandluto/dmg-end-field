...
官方数据与图片不再从 GitHub Release 读取。国内、海外站点各自托管同一份资源版本，客户端只访问当前站点的同源路径。
...
- 区域交付：国内站点提供图片 ZIP 分片；海外 Sites 为控制单次部署体积，只提供清单中的独立图片文件。桌面端先尝试分片，区域未部署分片时自动以 8 路并发下载同一批文件，最终都按逐文件 SHA-256 写入相同浏览器缓存。
