...
| 图片发包、数据发包、选择目录、打开目录 | `main.cjs:2283-2379` | 保留用途，按 Slim 格式重写 | 发包是开发工具，不应重新成为数据宿主 |
...
flowchart LR
  ...
  Workspace --> Cache["Service Worker + CacheStorage"]
...
- 启动时让 Service Worker 与安装包内的 `version.json` 对齐，自动激活已由用户安装的新桌面壳，但不下载另一份网站前端；
...
现有 `build-image-release-manifest.mjs` 输出 `assets-release-manifest.json`，而 Slim 读取的是 `web-image-manifest.json` 和浏览器图片包分片。旧脚本不能原样移植，应改为调用或复用 Slim 的图片清单/压缩包格式。
...
| 发包脚本 | 以 Slim manifest 为事实源重写无状态 wrapper |
...
- Electron 首次启动可以完成资料/图片初始化并进入工作台。
...
- 继续执行 Slim 现有 `npm run check`，包括真实离线 workspace 启动检查。
- 将同一份关键 UI/E2E 流程参数化，对 Web 和 Electron renderer 各跑一次。
- 新增桌面专项：静态宿主合同、WASM/ZIP MIME、重启持久化、图片缓存、占位无进程、发包工具 smoke。
- 最终进行 Windows 与 Mac 打包后手测。

## 十一、需要提前防住的风险

1. **不能改 origin。** 改 host、port 或 session partition 都会表现为“数据消失”。
2. **不能清 Chromium storage。** 旧图片更新代码中的 cache clear 不应移植。
3. **不能原样保留 Web 更新 UI，也不能完全忽略 Service Worker 版本。** 桌面端应显示桌面版本；安装新桌面包后自动让本地 SW 对齐安装包版本，但不提供网站远端前端更新。
4. **不能原样移植旧发包脚本。** 两套 manifest 已经不兼容。
5. **退出要有 flush 握手。** Electron 不管理数据库，但要给 renderer 完成最后一次持久化的机会。
6. **旧桌面数据不会自动进入 OPFS。** 升级时先保留旧 `userData` 文件、不主动删除；如果确有旧桌面用户数据需要继承，应另做一次性、只读迁移工具，不能因此长期保留双数据库。
...
真正需要适配的只有三处：稳定本地静态宿主、共享/桌面 Bootstrap、无状态发包入口。其他旧 Electron 能力大部分不是兼容资产，而是应该随旧前端一起退役的重复实现。
