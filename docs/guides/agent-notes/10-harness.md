...
通用 Agent 面对开放问题，临时组织这一轮需要的材料；垂直领域 Agent 反复处理同一类问题，因此可以持续复用领域知识、问题求解方法和 Tools，同时仍然根据每一次真实 Context 临场判断。

因此，垂直领域 Harness 的特色不是把流程写死，而是把领域知识、Tools、权限与审查机制稳定地组织在一起。Agent 仍然根据每一次真实 Context 临场判断，关键操作则可以被检查和追踪。
