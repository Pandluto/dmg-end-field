# 篇章 11：Harness

前面已经给 Agent 接上了 Typed Tools（类型化工具）。它能读取当前队伍的数据CRUD（增删改查），也能调用二次计算。

一个更难的问题，假如用户说：“帮我给当前角色搭一套适合当前队伍的装备。”

这是一个数学问题（排列组合数），还是一个知识问题（最简单粗暴的就是把什么都告诉AI）。一个游戏领域大神是这么做的？

why -> how。推理->表达。

我的干员需要什么？哪些装备、武器满足？给出解法。

这是**Agent完成一项真实业务** ，我当然可以为此专门写Tools（专项工具然后路由），但这和只会读稿的**血狼破军**有什么区别？

各位做题家此时可能反应过来了，这和做数学题有什么区别？

一个问题 -> 选择数学公式 -> 逻辑穿起来 -> 给出结论

没毛病，可惜AI不是你，你思考问题的时候只会想到“我在做题”不会想到那些“你是谁”，“你是干嘛的”，“我是读稿的”。

所以**血狼破军**做不出攻略，他的上下文是读稿。

那我把**血狼破军**的上下文改成做攻略不就可以了吗?

那可完蛋了，你得改什么呀？

Prompt（提示词：**血狼破军**是上交985）、

Host Prompt（宿主提示词：**血狼破军**是B站百万粉丝up主）、

Skill（技能包：**血狼破军**只会读稿）、

Tool Description（工具说明：**血狼破军**只会读稿）、

审批逻辑、事务状态等（骗你的，根本没人管他）

我们调教**血狼破军**（这里类比AI）是多维度，这里就需要引入harness

## 什么是 Harness？

Harness 是**围绕**模型建立的运行机制。

<div class="capability-strip" role="img" aria-label="模型、Harness 与真实环境的关系">
  <div><small>决定下一步</small><strong>模型</strong></div>
  <b>→</b>
  <div class="accent"><small>组织并约束执行</small><strong>Harness 系统</strong></div>
  <b>→</b>
  <div><small>产生真实结果</small><strong>Tools + 产品</strong></div>
</div>

不是单独一份 Prompt，也不是一组 Tools。它描述的是模型在什么信息和能力条件下工作，以及模型输出怎样被系统接收、执行、拒绝、记录和反馈。

这简直是**调教**。

我调教血狼破军吗？那太好了

Harness最窄的说法只包括 System Prompt（系统提示词）和 Tool Loop（工具循环）；

更宽的说法会把 Context（上下文）、Memory（记忆）、Skills（技能包）、状态、权限、沙箱、审批和结果检查都算进去。

不同文章使用同一个词时，谈论的可能不是完全相同的系统范围。

Harness直译是**控制**，工程化的问题，讲起来没有**调教**这么变态，

同一个模型接上不同 Harness，会表现得像不同的 Agent。差别不一定来自模型更聪明，而可能来自上下文、能力边界、状态、反馈和完成条件完全不同。

## 广义 Harness 会指什么？

Harness 没有一条被所有框架共同采用的边界。

从系统角度看，广义 Harness 可以理解为模型与外部环境之间的**运行支架**。它组织模型怎样获得信息、提出行动、接收反馈、延续状态，并把行动连接到真实执行。[AI Harness Engineering（AI Harness 工程）](https://arxiv.org/abs/2605.13357) 进一步把 Agent 能力放在“模型—Harness—环境”这个整体中讨论，而不是只归因于模型本身。

## Harness 和 Tools、Workflow 的关系

前面已经给血狼破军接上了 Tools。现在他有手了：能读取当前装备，能查装备数据，也能把一套方案真正应用到产品里。

但是 Tool 只回答一件事：**这个动作怎么做。**

`read_current_loadout` 负责读取当前配装，`apply_loadout` 负责应用一套配装。它们可以把参数、返回值、错误、副作用和权限要求都定义得很清楚，却不会替 Agent 回答：用户这次到底想听建议，还是想直接修改？应该先查什么？几套方案怎样比较？什么结果才算完成？

那把几个 Tool 串起来不就行了吗？

这就是 Workflow（工作流）擅长的事情。比如“用户确认 → 应用方案 → 重新读取页面”，这一段顺序会直接影响真实写入，当然应该固定。Workflow 定义的就是这种预先确定的顺序、分支、重试和结束条件。[LangGraph：Workflows and agents](https://docs.langchain.com/oss/python/langgraph/workflows-agents)

可是在形成方案以前，路不止一条。目标不清楚时要先追问；用户只想比较两套装备时，不需要进入写入；某条攻略不适用于当前队伍时，还要换证据重新判断。把这些也全部写成固定 Workflow，最后只会得到一张越来越大的流程图。

所以三者不在同一层：

- **Tool** 提供一个可以调用的动作；
- **Workflow** 固定那些必须固定的控制关系；
- **Harness** 组织一类完整业务的目标、方法、知识、能力边界和完成条件，再按当前问题使用 Tools，必要时引用 Workflow。

<div class="article-diagram relation-diagram" role="img" aria-label="配装 Harness、Tools 与 Workflow 的关系">
  <div class="diagram-heading"><small>RELATION</small><strong>一项配装业务怎样使用能力</strong></div>
  <div class="relation-root">
    <small>业务 HARNESS</small>
    <strong>定义一类配装问题的完整处理责任</strong>
    <span>目标 · 方法 · 知识 · 边界 · 完成条件</span>
  </div>
  <div class="relation-runtime"><span>由运行时加载并执行</span><i>↓</i></div>
  <div class="relation-branches">
    <div>
      <small>TYPED TOOLS</small>
      <strong>可以调用的动作</strong>
      <span>读取当前装备</span>
      <span>查询装备数据</span>
      <span>比较计算结果</span>
      <span>应用配装方案</span>
    </div>
    <div>
      <small>WORKFLOW</small>
      <strong>必须固定时的控制关系</strong>
      <span>用户确认</span>
      <i>→</i>
      <span>应用方案</span>
      <i>→</i>
      <span>重新读取</span>
    </div>
  </div>
</div>

同一个 Tool 可以被选人、配装、排轴等多个 Harness 复用。同一项业务里，也只有那些不能变化的顺序需要交给 Workflow；其余部分仍由模型根据用户意图和当前事实选择。

那配装方法本身放在哪里？写进 Skill 不就可以了吗？

## Harness 和 Skills 的关系

当然可以。配装攻略、比较方法、脚本和输出模板，本来就适合放进 Skill（技能包）。

但“把方法放进 Skill”和“由 Skill 负责这项业务”不是一回事。

| 对比 | Skill（技能包） | 业务 Harness |
| --- | --- | --- |
| 它是什么 | 可以复用的方法说明、脚本、模板和资源 | 一类完整业务的处理约定 |
| 它解决什么 | 告诉模型可以怎样做 | 说明这次业务要解决什么、使用什么、怎样结束 |
| 是否保存本次进度 | 通常不负责 | 需要知道目标、候选、确认和当前阶段 |
| 是否保证模型照做 | 自然语言方法不能保证 | 求解方法同样不能保证，硬边界另由程序执行 |

这里说 Skill 非强制，不是说宿主不能强制加载它。宿主完全可以规定“做配装前必须读取这个 Skill”，Skill 也可以携带能够执行的脚本。

问题在于，**读取了不等于照做了**。Skill 中的自然语言方法进入模型以后，仍然只是模型作决定时的一份输入；脚本之所以能够执行，也来自 Tool、运行时或沙箱提供的能力。

Skill 适合保存“这类事情通常怎样做”。Harness 可以使用这些材料，但还要承担本次业务的目标、状态、能力边界和完成责任。

方法有了，本次配装需要的当前装备、攻略和用户确认又从哪里来？

## Harness 和 Context 的关系

这就回到了前面那个问题：把血狼破军的 Context（上下文）改成做攻略，不就行了吗？

先得说清楚，Context 不是某一个文件，也不只是聊天记录。它指的是**当前这一刻，代码或者模型能够使用的信息**。

当前装备来自真实产品，装备数值来自数据目录，攻略来自知识库，用户刚才确认了哪套方案则属于本次业务任务。这些信息的来源不同、保存时间不同，也不是全部都应该交给模型。

OpenAI Agents SDK 因此区分 Local Run Context（本地运行上下文）和 Model-visible Context（模型可见上下文）：身份、权限和任务引用可以只供 Tools、Hooks（钩子）与应用代码使用；真正进入模型请求的，只是当前判断所需的那一部分信息。[OpenAI Agents SDK：Context management](https://openai.github.io/openai-agents-python/context/)

<div class="article-diagram context-diagram" role="img" aria-label="事实、本次任务和会话怎样被运行时组装成本地上下文与模型可见上下文">
  <div class="diagram-heading"><small>CONTEXT FLOW</small><strong>信息从哪里来，最后给谁使用</strong></div>
  <div class="context-flow">
    <div class="context-sources">
      <small>模型调用之外保存的信息</small>
      <span><b>权威产品事实</b>当前装备、装备数据、公式及其版本</span>
      <span><b>带来源和条件的领域知识</b>攻略原文或检索片段</span>
      <span><b>本次业务任务</b>目标、待确认方案、确认状态、方法版本</span>
      <span><b>会话记忆</b>对话、Tool 调用与 Tool 结果历史</span>
    </div>
    <i class="diagram-arrow">→</i>
    <div class="context-runtime">
      <small>HARNESS 运行时</small>
      <strong>绑定 · 选择 · 压缩 · 刷新</strong>
      <span>决定当前阶段真正需要什么</span>
    </div>
    <i class="diagram-arrow">→</i>
    <div class="context-outputs">
      <span><b>本地运行上下文</b>身份、权限、任务引用、写入范围<br><em>只供代码使用</em></span>
      <span><b>模型可见上下文</b>当前目标、必要事实、对话片段、可用 Tools<br><em>进入本次模型调用</em></span>
    </div>
  </div>
  <div class="diagram-feedback"><b>Tool 结果</b><i>→</i><span>成为新的读取证据；有副作用时更新产品状态或任务状态</span><i>→</i><b>运行时重新组装</b></div>
</div>

Harness 负责声明这类问题需要什么信息；事实源负责保存信息；运行时负责在每一轮选择、压缩和刷新。Context 是这次真正组装出来的结果。

信息选出来以后，哪些应该写给模型，哪些必须留在代码侧检查？这才轮到 Prompt。

## Harness 和 Prompts 的关系

那把目标、攻略、方法和禁止事项都写进 Prompt，不就结束了吗？

Prompt 确实重要。它可以告诉模型自己正在处理什么问题、应该参考什么方法、怎样解释结果，也可以通过 Instructions（指令）建立模型侧的优先级。

但这里有两个经常被混在一起的东西：

| 名词 | 指什么 |
| --- | --- |
| Prompt Template（提示词模板） | 可以反复使用和独立版本化的角色、方法、示例与表达模板 |
| Final Model Input（最终模型输入） | Runtime 针对这一次调用，实际交给模型的指令、对话、Tool 结构、结果和证据 |

模板只是最终输入的一部分。对话历史、检索片段和 Tool 结果最后也会进入模型，却不会因此都变成 Prompt 模板。

更重要的是，Prompt 只能影响模型怎样判断，不能替代代码侧权限。Prompt 里写一百遍“没有确认不得修改”，模型真正发起 `apply_loadout` 时，执行层仍然要再检查一次。否则所谓禁止，只是一句希望模型听话的话。

所以 Prompt 负责把问题和方法交给模型；Tool、权限、审批和产品代码负责决定真实行动能不能发生。

绕了一圈，我们终于可以回答本项目真正要做的 Harness 了：哪些东西交给模型判断，哪些东西必须由程序保证？

## 本项目 Harness 的特性

本项目不是把所有规则打成一个包，再统一改名叫 Harness。

这里真正要分开的是两类完全不同的东西。

第一类是**求解方法**：面对一项配装问题，通常要理解什么、参考哪些知识、怎样形成和比较候选、什么时候应该追问。它告诉模型怎样更像一个会解决问题的人。

第二类是本文使用的项目术语——**硬边界**：哪些 Tool 可以出现、谁能写哪些字段、用户是否确认、待确认方案是否已经过期、写入以后真实页面是否发生了对应变化。这些事情不能靠模型自觉，必须由程序检查。

所以本项目里的 Harness 同时走两条线：

- 求解方法是**强参考**，要求模型朝正确的方法和产物工作，但不规定唯一的内部思考顺序；
- 产生真实后果的边界由程序**强制执行**，模型不能自行跳过。

这套 Harness 面向的是配装、排轴这类完整业务，而不是某个 Tool 或某件装备；它的结束也不是模型说一句“已经完成”，而是产品中的真实结果能够被重新读取和验证。

### 为什么不能强制？

那为什么不把求解方法也写成严格的状态机？

因为用户说“帮我搭一套装备”，可能只是想听搭配原理，也可能正在比较两套方案，或者已经准备应用其中一套。目标不清楚时要先追问，事实充分时可以直接分析，某条攻略不适用时还要换一种证据。

同一个问题存在多条合理路径。真正需要稳定的是它不能漏掉哪些业务责任，而不是模型必须先读 A、再读 B、最后按照唯一顺序思考。

模型的内部推理也不是 Runtime 可以逐句检查的程序。硬把自然语言方法伪装成强制 Workflow，只会让系统遇到例外就开始卡死。

<div class="article-diagram dual-track-diagram" role="img" aria-label="配装求解方法可以灵活调整，但所有真实写入都必须通过硬边界">
  <div class="diagram-heading"><small>METHOD × CONTRACT</small><strong>方法可以变化，产生真实后果的边界必须通过</strong></div>
  <div class="diagram-track flexible">
    <div class="track-label"><small>求解方法</small><strong>强参考</strong><span>允许跳过、返回和重新规划</span></div>
    <div class="track-nodes">
      <span>理解目标</span><i>↔</i><span>查看当前装备</span><i>↔</i><span>查知识与计算</span><i>↔</i><span>比较装备组合</span><i>↔</i><span>追问或解释</span>
    </div>
  </div>
  <div class="diagram-track enforced">
    <div class="track-label"><small>运行时硬边界</small><strong>程序强制</strong><span>任何写入都不能绕过</span></div>
    <div class="contract-flow">
      <div class="contract-phase">
        <small>写入前硬检查</small>
        <div class="contract-nodes"><span>模型提出写入</span><i>→</i><span>Tool 与参数允许？</span><i>→</i><span>用户确认有效？</span><i>→</i><span>方案与产品版本仍有效？</span><i>→</i><span>写入范围允许？</span></div>
      </div>
      <i class="contract-down">↓</i>
      <div class="contract-phase">
        <small>执行与完成验证</small>
        <div class="contract-nodes"><span>执行写入</span><i>→</i><span>重新读取产品状态</span><i>→</i><span>与确认方案一致？</span></div>
      </div>
    </div>
  </div>
</div>

图中上半部分可以来回调整，下半部分一步也不能绕过。

程序能够检查证据是否存在、版本是否正确、写入结果是否符合后置条件；至于这些证据能不能推出一套好方案，仍然要由模型结合领域知识判断。

那么，是谁把这两条线放进同一次配装任务里持续运行？这就是 Runtime。

## Runtime 在整个 Agent 中扮演什么角色？它和 Harness 是什么关系？

一份 Harness 声明不会自己跑起来。

单次模型调用只会产生一次输出，但完整配装可能经历多次读取、追问、计算、确认、写入和重新验证。中间还可能暂停，等用户过一会儿再回来继续。

负责持续协调这些过程的应用程序，就是 Harness Runtime（Harness 运行时）。

业务 Harness 版本说明“这类问题应该怎样处理”；Runtime 则创建或恢复本次任务，组装当前信息和可用 Tools，把模型提出的行动交给程序检查，执行以后保存结果，再决定继续、等待还是结束。

OpenAI Agents SDK 已经提供模型与 Tool 之间的基础循环、会话记忆和审批恢复；本项目的 Runtime 还要知道配装进行到哪里、当前使用哪一版方法、用户确认了哪套方案，以及修改后是否真的生效。[OpenAI Agents SDK：Running agents](https://openai.github.io/openai-agents-python/running_agents/)

<div class="article-diagram runtime-diagram" role="img" aria-label="业务 Harness 版本如何由运行时变成一次可以暂停恢复并验证结果的配装任务">
  <div class="diagram-heading"><small>HARNESS RUNTIME</small><strong>声明怎样变成一次真实业务过程</strong></div>
  <div class="runtime-layout">
    <div class="runtime-input">
      <small>业务 HARNESS 版本</small>
      <strong>声明这类配装怎样处理</strong>
      <span>目标</span><span>求解方法</span><span>所需知识</span><span>可用能力</span><span>完成条件</span>
    </div>
    <i class="diagram-arrow">→</i>
    <div class="runtime-core">
      <small>HARNESS 运行时</small>
      <span>创建或恢复本次配装任务</span>
      <i>↓</i>
      <span>组装本轮信息与可用 Tools</span>
      <i>↓</i>
      <span>模型提出下一步</span>
      <i>↓</i>
      <span>检查 Tool、参数、权限、审批与版本</span>
      <i>↓</i>
      <span>执行 Tool，并把结果交回模型继续判断</span>
      <i>↓</i>
      <span>保存进度、等待确认或继续处理</span>
    </div>
    <i class="diagram-arrow">→</i>
    <div class="runtime-output">
      <small>真实产品</small>
      <strong>重新读取并验证</strong>
      <span>符合确认方案<br><b>完成</b></span>
      <span>不符合或状态已变<br><b>进入恢复或重新规划</b></span>
    </div>
  </div>
  <div class="runtime-state"><b>本次业务任务保存</b><span>Harness 版本</span><span>产品版本</span><span>待确认方案</span><span>用户确认</span><span>当前阶段</span></div>
</div>

Runtime 还要留下 Execution Trace（执行轨迹，后文简称 Trace）：这次用了哪版 Harness、看到了什么产品状态、调用过哪些 Tools、用户确认了什么、最后拿什么证明完成。

到这里，一套方法已经能够变成一次真实、可暂停、可检查的业务过程。接下来的问题是：为什么垂直领域尤其值得长期维护这套东西？

## 典型的垂直领域 Harness

通用 Agent 当然也能解决真实问题。编码 Agent 可以改文件、跑命令、验证测试，产生的副作用一点也不比游戏 Agent 小。

垂直领域真正特殊的地方，是**同一类问题会反复出现**。

用户今天要配装，明天还会要配装；角色、装备、事实来源和完成条件并不会每次从零发明。既然问题结构相对稳定，就值得把它沉淀成长期维护的业务能力。

### 1、垂直领域的特性：解决实际问题

区别不在“谁能解决实际问题”，而在什么东西可以提前沉淀下来。

| 对比 | 通用 Agent | 垂直领域 Agent |
| --- | --- | --- |
| 问题 | 范围开放，结构通常由用户临时给出 | 问题集合相对稳定，可以提前建模 |
| 对象与事实 | 按任务临时连接文件、网页、代码或服务 | 可以预先确定领域对象和权威事实来源 |
| 方法 | 根据任务临时规划 | 同类问题可以积累可复用的求解方法 |
| 写入与完成 | 每次任务临时决定 | 可以沉淀领域写入范围和完成条件 |
| 主要迭代 | 通用能力、路由和任务适应性 | 领域知识、业务方法、运行状态和领域评测 |

Multi-agent（多 Agent）讨论的是几个 Agent 怎样分工；业务 Harness 讨论的是一项领域工作究竟由谁负责、依赖什么、怎样完成。多叫几个人过来，不会自动补齐缺失的领域建模。

### 2、以本项目为例

继续看“帮我给当前角色搭一套适合当前队伍的装备”。

选出几件装备，就算完成配装了吗？

显然不算。几件装备放在一起，只是一个**候选装备组合**。它还要绑定目标角色、队伍条件、用户限制、选择理由，以及形成方案时读取到的真实配装，才能成为一份可以交给用户确认的**待确认配装方案**。

从用户的一句话到最终真实生效，中间会遇到这些分支：

<div class="article-diagram business-flow-diagram" role="img" aria-label="一次装备搭配业务可以解释、追问、推荐、确认、应用或重新规划">
  <div class="diagram-heading"><small>BUSINESS FLOW</small><strong>一项装备搭配业务实际怎样分支</strong></div>
  <div class="flow-step start"><small>用户请求</small><strong>给当前角色搭一套装备</strong></div>
  <i class="diagram-down">↓</i>
  <div class="flow-decision">
    <strong>用户只是询问搭配原理？</strong>
    <div><span class="branch-stop"><b>是</b>解释组合规则后结束，不写入</span><span><b>否</b>继续形成具体方案</span></div>
  </div>
  <i class="diagram-down">↓</i>
  <div class="flow-decision">
    <strong>目标角色、队伍条件和搭配目标清楚吗？</strong>
    <div><span class="branch-loop"><b>否</b>追问，补齐目标与限制</span><span><b>是</b>读取当前装备与必要知识</span></div>
  </div>
  <i class="diagram-down">↓</i>
  <div class="flow-build">
    <small>按当前任务需要读取</small>
    <span>当前装备</span><span>装备数据</span><span>必要时：适用攻略</span><span>必要时：公式计算</span>
    <b>↓</b>
    <strong>候选装备组合 → 待确认配装方案</strong>
  </div>
  <i class="diagram-down">↓</i>
  <div class="flow-decision">
    <strong>用户确认应用这套方案吗？</strong>
    <div><span class="branch-loop"><b>否</b>修改候选、保留建议或结束</span><span><b>是</b>进入写入前检查</span></div>
  </div>
  <i class="diagram-down">↓</i>
  <div class="flow-decision">
    <strong>等待确认期间，真实配装发生过变化吗？</strong>
    <div><span class="branch-loop"><b>是</b>旧方案过期，重新读取和规划</span><span><b>否</b>应用方案并重新读取页面</span></div>
  </div>
  <i class="diagram-down">↓</i>
  <div class="flow-finish"><span><b>不一致</b>报告失败、恢复或重新规划</span><strong>真实页面与确认方案一致</strong><span><b>一致</b>留下证据并完成</span></div>
</div>

这张图不是一条要求模型逐格执行的 Workflow。它画的是一项配装不能丢掉的业务责任：目标要弄清楚，事实要有来源，候选要形成完整方案，修改要确认，写入要受限，结果要重新验证。

至于这一次先追问还是先读取、是否需要计算、要不要进入写入，仍然由模型根据实际问题判断。

这种“一类问题通常怎样使用知识形成结果”的东西，其实早就有一个名字。

### 3、引入问题求解方法 PSM

这不是 Agent 时代才发明的问题。

知识工程把“一类任务通常怎样使用领域知识得到结果”称为 Problem-Solving Method（**问题求解方法，PSM**）。它把任务、求解方法和领域知识分开，专门研究某一类知识怎样参与求解。[知识工程对 PSM 的介绍](https://www.cs.vu.nl/~guus/papers/Schreiber07a.pdf)

用到本文里，几个名词可以这样理解：

| 知识工程概念 | 中文 | 在本文中指什么 |
| --- | --- | --- |
| Task / Task Model | 任务 / 任务模型 | 一类领域问题负责什么 |
| Problem Instance | 问题实例 | 本次具体目标、输入和限制 |
| Problem-Solving Method | 问题求解方法 | 这类问题通常怎样使用知识形成结果 |
| Domain Knowledge | 领域知识 | 求解方法使用的游戏事实、攻略、装备数据目录和公式 |

这个概念提醒我们：配装不是一串 Tool 调用；攻略和装备数据也不是配装方法，它们是方法在求解时使用的领域知识。

PSM 在本文中只解释 Harness 里的**求解方法层**。它不负责模型循环、Context、Tools、权限、运行状态和真实结果验证，因此不能拿来替代 Harness。

方法有名字了，接下来还缺真正让它得出结论的游戏知识。

## 垂直领域的知识（游戏知识）

有了配装方法，不代表 Agent 已经知道当前角色需要什么。

它还要读取真实配装、装备数值、攻略和计算结果。那这些东西是不是也应该全部写进 Harness？

### 1、游戏知识和 Harness 的关系

不应该。

Harness 使用知识，但不应该复制整个装备数据库，也不应该把某一篇攻略当成永远正确的结论写死。方法和知识会以不同速度变化，把它们粘在一起，只会让其中一个更新时另一个被迫重新发布。

| 信息 | 权威来源 | 在业务中的角色 |
| --- | --- | --- |
| 当前队伍、配装、排轴与 BUFF | 当前打开的方案和真实产品页面 | 这次问题的真实起点 |
| 武器、装备、技能和 BUFF 数值 | 装备数据目录 | 可核对的游戏事实 |
| 角色定位、配装建议和适用场景 | 带来源与条件的攻略知识 | 候选生成与解释依据 |
| 战斗约定和统计口径 | 用户条件、项目约定 | 约束比较范围 |
| 伤害与统计结果 | 公式和计算引擎 | 可复现的计算证据 |
| 历史选择与用户修正 | 本次业务任务 | 这次问题的决策上下文 |

Harness 真正要声明的是：解决这类问题需要哪些知识，它们从哪里读取，在推理中扮演什么角色，适用条件怎样记录，以及事实变化以后旧方案是否失效。

模型可以负责解释，事实仍要回到各自来源。当前配装由产品读取，装备数值来自数据目录，策略结论保留攻略来源和适用条件，计算结果来自公式引擎。文字可以把证据讲明白，不能替代证据本身。

最简单粗暴的办法，当然还是把这些东西都塞进 Prompt。

### 2、问题：写死 Prompts？实际遇到的迭代问题

项目早期这样做完全没问题。流程还没有稳定时，一段 Prompt 就能快速验证模型到底听没听懂。

问题出现在规则越来越多以后。

例如“应用配装前必须经过用户确认”这句话，很可能同时出现在固定 Prompt、宿主提示词、总 Skill、Tool Description（工具说明）、Runtime 和产品代码里。

这时问题已经不是 Prompt 太长，而是**同一个业务决定有多个主人**。

<div class="article-diagram ownership-diagram" role="img" aria-label="应用配装前必须确认这条规则，从多个重复副本重构为不同层次各自负责">
  <div class="diagram-heading"><small>BEFORE × AFTER</small><strong>“应用配装前必须确认”应该由谁负责</strong></div>
  <div class="ownership-columns">
    <div class="ownership-before">
      <small>重构以前 · 一句话复制多遍</small>
      <strong>多个副本，彼此可能冲突</strong>
      <div><span>固定 Prompt</span><span>宿主提示词</span><span>总 Skill</span><span>Tool 说明</span><span>运行时</span><span>产品代码</span></div>
    </div>
    <i class="diagram-arrow">→</i>
    <div class="ownership-after">
      <small>重构以后 · 不同责任各有主人</small>
      <span><b>业务 Harness</b>声明什么时候需要确认</span>
      <span><b>本次任务</b>保存确认的是哪套方案及其产品版本</span>
      <span><b>运行时</b>检查确认是否存在、方案是否仍然有效</span>
      <span><b>Tool / 产品</b>无有效确认时拒绝真实写入</span>
      <span><b>执行轨迹</b>记录确认、检查、写入与验证</span>
    </div>
  </div>
</div>

重构也不是给这句话换一个新文件继续复制。

业务 Harness 声明什么时候需要确认；本次任务保存用户确认的是哪套方案；Runtime 检查这次确认是否仍然有效；Tool 和产品代码在条件不满足时拒绝写入；Trace 则证明这些事情到底有没有发生。

开发者真正关心的是“应用以前是否真的确认了”，代码却按照 Prompt、Skill、Tool、Runtime 等文件和模块分散保存。[Harness Handbook（Harness 手册）](https://ruhan-wang.github.io/Harness-Handbook/#one-behavior-many-implementation-sites) 讨论的正是这种问题：从一个可观察的行为出发，找到共同产生它的所有实现位置。

文件树告诉我们代码放在哪里；Harness 和 Trace 则让我们从一项业务行为反过来找到这些代码各自负责什么。

### 3、Typed Tools 完成之后，迭代的是什么？

所以 Typed Tools 做完，项目并没有做完。

Tools 稳定，只表示 Agent 已经拥有相对稳定的手。后面真正持续变化的，是它怎样使用这些手解决业务。

| 发生了什么变化 | 应该修改哪里 | 不应该主要修改哪里 |
| --- | --- | --- |
| 产品新增一种读取或写入能力 | Typed Tool 与产品接口 | 全局 Prompt |
| Tool 参数、返回值或错误合同变化 | Tool 参数结构 / Tool 实现 | 业务攻略 |
| 游戏数据或公式变化 | 装备数据目录 / 公式引擎 | 业务 Harness 的求解方法 |
| 攻略结论和适用条件变化 | 领域知识（Domain Knowledge） | 工具说明 |
| 一类业务通常怎样分析、追问和形成候选 | 业务 Harness 版本 | 固定 Agent Prompt |
| 本次任务、阶段和版本怎样运行 | Harness Runtime（运行时） | Skill |
| 权限、审批、版本一致性或写入范围变化 | Harness Runtime / 产品执行层 | Prompt 中的警告 |
| 怎样判断系统是否做对 | Trace、合同测试与真实界面（UI）验证 | 运行时教学 |

Prompt 当然仍然会迭代，只是不再充当所有业务变化的垃圾桶。

以后调整配装方法，改配装 Harness；攻略结论变化，改知识；产品增加一种动作，改 Tool；审批和版本一致性变化，改 Runtime 或产品执行层。

只有把变化放回正确的归属处，系统才能回答两个重要问题：

- 这次行为为什么变了？
- 我只改配装，会不会意外改变选人、排轴或 BUFF？

要回答这两个问题，接下来就不能只整理文件了，必须开始划分业务边界和版本。

## 拆解业务 → 建模：从方法走向可迭代体系

当方法、知识和执行责任各自归位以后，下一个问题不再是“代码放进哪个目录”。

真正要决定的是：什么东西可以独立负责一项业务，独立修改，又不会把其他业务一起带崩。

先解决同一类业务怎样换版本，再解决不同业务之间怎样划边界。

### 1、迭代与管理

先看一个最实际的问题。

配装方法 V1 已经给用户形成了一套待确认方案，用户暂时离开了。第二天，配装方法 V2 发布。等用户回来点击确认时，这项正在进行的任务能不能偷偷换成 V2？

不能。等在页面上让用户确认的，是 V1 形成的方案，不是系统后来换掉规则以后重新理解出来的另一套东西。

这就逼着我们分清三种记录：

| 记录 | 稳定性 | 负责什么 | 例子 |
| --- | --- | --- | --- |
| 业务定义（Business Definition） | 相对稳定 | 业务边界、支持动作、最大 Tool 范围、写入范围和完成条件 | 配装可以改装备，不能改队伍成员 |
| 业务 Harness 版本（Business Harness Revision） | 单个版本不可变；通过发布新版本持续迭代 | 业务方法、阶段、知识入口、Tool 引用、异常处理和表达方式 | 配装 V1、V2 |
| 业务事务（Business Transaction） | 每次工作独立产生 | 目标、限制、当前阶段、证据、待确认方案、审批、产品版本和运行中的 Harness 版本 | 正在为当前角色准备一套配装 |

它们分别回答三个问题：

- 这项业务究竟负责什么？
- 当前发布的方法是什么？
- 这一次具体工作进行到哪里？

这里的 Business Transaction（业务事务）不是数据库事务，而是一次可以跨回合暂停和恢复的业务任务。它保存目标、证据、待确认方案、用户确认、产品版本以及本次固定使用的 Harness 版本。

<div class="article-diagram version-timeline-diagram" role="img" aria-label="配装 Harness 发布并启用新版本以后，已经开始的任务继续使用原版本，新任务使用新版本">
  <div class="diagram-heading"><small>VERSION TIMELINE</small><strong>发布新方法，不会偷偷替换正在进行的任务</strong></div>
  <div class="version-track">
    <div><small>时间 01</small><strong>配装方法 V1 发布并启用</strong><span>成为新任务的默认版本</span></div>
    <i>→</i>
    <div class="transaction-a"><small>时间 02</small><strong>配装任务 A 开始</strong><span>固定使用 V1</span></div>
    <i>→</i>
    <div><small>时间 03</small><strong>配装方法 V2 发布</strong><span>启用为新任务的默认版本；任务 A 仍使用 V1</span></div>
    <i>→</i>
    <div class="transaction-b"><small>时间 04</small><strong>配装任务 B 开始</strong><span>使用 V2</span></div>
  </div>
  <div class="version-revoke"><b>如果 V1 必须撤销</b><span>任务 A 应停止、迁移或重新规划，不能静默切换方法。</span></div>
</div>

同一个宿主会话可以先选人、再配装、然后排轴。每项新业务使用当前启用的版本；已经开始的任务则继续使用自己的版本，不能在用户确认以后偷换规则。

版本解决的是同一类业务怎样迭代。接下来还要回答：一项业务到底应该大到什么程度、小到什么程度？

### 2、形象的说法：原子化 Harness

说到“原子”，最容易产生的误解就是越小越好。

一个 Tool 很小，一个字段更小，一件装备也很小。但用户不会把“调用一次 `read_current_loadout`”当成一项完整工作，系统也不能靠读完一次数据就宣布配装完成。

这里的原子不是最小文件、最小 Tool、最小实体或最小步骤。它指的是一类能够**独立负责、独立版本化、独立验证完成**的业务。

判断一个边界是否适合作为原子 Harness，可以看五件事：

1. 用户是否会把它当作一件完整工作提出；
2. 它是否拥有明确的输入、输出和完成条件；
3. 它是否有相对独立的求解方法与知识角色；
4. 它是否拥有明确的直接写入范围或只读责任；
5. 它是否值得独立启用、回退和撤销版本。

按这个标准，本项目得到五个业务 Harness：

| 原子 Harness | 完整业务责任 | 直接写入范围 | 完成证据 |
| --- | --- | --- | --- |
| 选人 | 查看、增加、删除、替换和调整队伍 | 队伍成员与顺序 | 真实页面中的当前队伍 |
| 配装 | 分析、推荐、比较、预览和应用角色配置 | 武器、装备、技能等级与配置输入 | 真实角色配置页 |
| 排轴 | 查看、添加、移动、替换和应用技能按钮 | 按钮身份、位置、顺序与排轴结构 | 真实时间轴 |
| BUFF | 查询、添加、替换和批量处理 BUFF | BUFF 绑定、层数与相关战斗状态 | 真实按钮 BUFF 状态 |
| 计算统计 | 计算、比较、归因、诊断和解释 | 不直接改前四类状态 | 绑定方案与公式版本的结果 |

某种套装效果只是配装时使用的知识或限制；角色和单件装备只是业务操作的对象；几件装备放在一起，也只是一个候选组合。

真正值得独立建成 Harness 的，是“为角色分析、推荐、确认并应用整套装备”这项用户能够完整提出、系统也能够完整交付的工作。

同样，`read_current_loadout` 和 `apply_loadout` 不是两个业务 Harness。它们是同一项配装业务在不同阶段使用的 Tools。

原子化保护的是业务责任和迭代生命周期，不是把系统切成尽可能多的小块。

### 3、业务拆分？No，是领域问题求解 Harness 及其可迭代体系

所以建五个目录不等于建成五个 Harness。

每个业务 Harness 都要拥有自己的方法版本、知识来源、业务事务、写入边界、失败恢复方式、完成验证和执行证据。

五个业务 Harness 共享 Tools、知识来源和同一份产品状态，但不共享模糊的业务所有权。

业务边界也不等于物理文件边界。BUFF 字段即使存放在时间轴按钮对象里，排轴业务仍只负责按钮身份、位置和顺序，BUFF 业务只负责 BUFF 状态。数据住在一起，不等于业务责任也要混在一起。

用户说“换人、配装、排轴、上 BUFF 后计算”时，也不需要再造一个万能 Harness。管理器只负责保存跨业务计划和依赖关系，再分别创建对应的业务事务。

<div class="article-diagram harness-ecosystem-diagram" role="img" aria-label="管理器把跨业务请求拆成五个独立业务 Harness，它们共享工具、知识和产品状态，但各自拥有写入范围与完成验证">
  <div class="diagram-heading"><small>ATOMIC BUSINESS HARNESSES</small><strong>共享能力，不共享模糊的业务责任</strong></div>
  <div class="ecosystem-manager"><small>管理器</small><strong>按用户请求和依赖关系创建一个或多个业务任务</strong></div>
  <div class="ecosystem-dependency"><span>下面是一次组合请求的依赖关系</span><i>↓</i></div>
  <div class="ecosystem-harnesses">
    <div><small>01</small><strong>选人</strong><span>成员与顺序</span></div>
    <i>→</i>
    <div><small>02</small><strong>配装</strong><span>角色配置</span></div>
    <i>→</i>
    <div><small>03</small><strong>排轴</strong><span>时间轴结构</span></div>
    <i>→</i>
    <div><small>04</small><strong>BUFF</strong><span>战斗状态</span></div>
    <i>→</i>
    <div><small>05</small><strong>计算</strong><span>只读并统计</span></div>
  </div>
  <div class="ecosystem-contracts"><span>各自的方法版本</span><span>各自的业务事务</span><span>各自的写入范围</span><span>各自的完成验证</span></div>
  <div class="ecosystem-shared"><small>共同基础</small><span>Typed Tools</span><span>游戏知识与公式</span><span>同一份产品状态</span><span>执行轨迹</span></div>
  <div class="ecosystem-stop"><b>前一步失败、被拒绝或过期</b><span>依赖该结果的后续任务不得继续使用旧方案</span></div>
</div>

这不是五份更长的 Prompt，而是五项可以独立运行、独立验证、独立版本化的领域业务，以及管理它们持续演进的体系。

## 开发与训练（未完待续）

回到最开始的血狼破军。

他做不出攻略，不一定是因为他不够聪明。也可能是没有读取数据的 Tool、拿错了游戏事实、缺少配装方法、忘了用户确认的方案、越过了写入边界，或者写完以后根本没有检查产品是否真的变化。

训练改变模型本身；Harness 系统开发改变模型在什么信息、能力、状态和约束中工作。

当 Agent 做错一件事时，应该先判断错误发生在哪一层：

| 问题 | 更可能需要修改 |
| --- | --- |
| 模型普遍无法理解某种语言或稳定完成基础推理 | 模型、训练或推理能力 |
| 缺少一项真实读取、计算或写入能力 | Typed Tool / 产品接口 |
| 游戏事实错误或已经过期 | 装备数据目录、公式或领域知识 |
| 一类业务经常漏查证据、候选不完整或不会处理修正 | 业务 Harness 版本 |
| 模型能绕过审批、越界写入或使用已经过期的待确认方案 | Harness 运行时 / 产品执行层 |
| 系统说完成，但真实页面没有变化 | 完成验证与 Trace |

所以不能把所有失败都归因于模型不够强，也不能把所有失败都继续堆进 Prompt。

一个更强的模型可以提高判断质量；一个更成熟的 Harness 系统，则让它在正确的事实、能力和边界中判断，并让结果可以验证、追溯和修改。

垂直领域 Agent 最终交付的不是某个模型单独的聪明程度，而是模型、Harness 系统与真实产品共同形成的业务能力。
