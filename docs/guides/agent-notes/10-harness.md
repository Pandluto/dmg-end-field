...
它能读取当前队伍和配装，能对装备、排轴和 BUFF 做 CRUD（增删改查），也能调用真实公式完成计算。

一个更难的问题。假如用户说：

> 给我干员搭配一套适合的装备。

这是一个数学问题吗？把所有装备做排列组合，找出数值最高的那一套。

还是一个知识问题？最简单粗暴的办法，就是把游戏资料全部告诉 AI。

但一个游戏领域大神是怎么做的？

他会先问为什么：这个干员需要什么，队伍里缺什么，用户想解决什么。然后才进入怎么做：哪些装备、武器和属性满足条件，怎样比较，最后怎样把解法讲清楚。

**Why → How。推理 → 表达。**

显然，这句话不只要调用一个基础 Tool（工具）。

这是 **Agent 完成一项真实业务**。

我当然可以为它写一个专门的 Tool，再把这类请求路由过去。

但 Agent 学会的只是调用程序，只能解决被我提前写进程序里的那个问题，并没有学会前因后果，也不会举一反三。

我必须用 Context（上下文）告诉 Agent 当前发生了什么、这类问题通常怎样处理。

同一个 Tool Description（工具描述），在不同语境下可能也要承担不同的提示作用。权限、审批逻辑、事务状态和真实产品状态，也要一起为 Agent 服务。
...
它不是单独一份 Prompt，也不是一组 Tools。

它描述的是模型在什么信息和能力条件下工作，以及模型输出怎样被系统接收、执行、拒绝、记录和反馈。

Harness 直译更接近“挽具”或“线束”。放在这里，可以理解成把模型接住，再把它控制在工程边界里。这个词很工程化，讲起来没有“调教”那么变态。
...
## 面向业务的 Harness：先从“解一道题”说起

一道普通证明题，解题的人要看懂题目，想起可用的定理，选择证明办法；走不通，就换一条路。
...
Agent 的运行看起来也很像：理解用户的话，取得需要的信息，调用 Tools，处理返回结果；需要 Tool 就继续循环，不再需要就退出循环并给出答案。

PSM 让这件事变得好懂。但是：

> **PSM 没有智能；“怎样解一道题”的方法可以写，但不能把每一步写死。**

Harness 要管理的不是一份固定答案，而是模型处理这一类问题时可以参考的方法和条件。
