...
假如用户说：“帮我给当前角色搭一套适合当前队伍的装备。”

一次配装不是挑选某一件装备，而是根据目标角色、当前队伍、装备槽位和套装效果，把几件装备组合成一套完整方案。Agent 应该先读取角色现在穿什么，还是先查装备数据和攻略？它怎样比较几套装备组合，而不是逐个槽位孤立修改？用户只是想听建议时，它会不会直接写入？用户确认以后，它应用的还是不是刚才确认的那套配装方案？Tool 返回成功以后，它会不会重新读取真实页面，检查方案是否真的生效？
