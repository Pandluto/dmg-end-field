...
一个游戏领域大神会怎么做？ 我的干员需要什么？哪些装备、武器满足？给出解法。
...
我的 Tool Description（工具描述）在不同语境下也会不一样，
...
Harness直译是**驾驭**，工程化的问题，讲起来没有**调教**这么变态
...
后来我们发现，只把关键规则放在 Skill 里并不可靠：它不一定会在需要的那一轮进入模型的 Context。
...
Typed Tools 让 Agent 的“手”相对稳定下来，之后长期变化的是它在不同位置拿到的 Context：方法、知识、Tool 的用途、Tool Result 的解释和审查规则都会继续更新。

只要来源、用途、触发时机和退出条件清楚，这些变化就不必重写一份总 Prompt。
...
DEF OpenCode 也在探索一套 Context 管理的注册机制；相关实践尚未沉淀为严谨的方法论，本文暂不展开。
