...
2026 年，OpenAI 在介绍 Agents SDK 的演进时，直接把重点放在 Agent 循环的 Harness 上；Anthropic 则把 Harness 解释成调用模型并路由 Tool 的循环，同时明确把它与会话记录、执行沙箱分开。[OpenAI：The next evolution of the Agents SDK](https://openai.com/index/the-next-evolution-of-the-agents-sdk/) · [Anthropic：Scaling Managed Agents](https://www.anthropic.com/engineering/managed-agents)

两家的系统边界并不相同，但都在说明同一件事：Agent 的表现不只取决于模型，还取决于模型外面那套让它持续工作的工程机制。

先把 **Harness** 这个名词放在这里，回到本项目。

前面已经给 Agent 接上了 Typed Tools（类型化工具）。它能对当前队伍的数据做 CRUD（增删改查），也能调用二次计算。

现在用户说：“帮我给当前角色搭一套适合当前队伍的装备。”

这是一个数学问题，还是一个知识问题？最简单粗暴的做法，是穷举装备组合，或者把所有游戏知识都告诉模型。但真正熟悉游戏的人通常不会这样做。他会先判断：

- 我的干员在当前队伍里需要什么？
- 哪些装备和武器满足这些需要？
- 为什么这套组合比另外几套更合适？

这是从 **why 到 how**：先形成判断，再组织证据，最后把解法讲清楚。

这也是 **Agent 完成一项真实业务**。我当然可以专门写一个“自动配装 Tool”，再把用户请求路由过去。这样交付很快，但求解方法已经被写进 Tool：Agent 没有学会处理配装，只是调用了一段提前写好的配装程序。

这并不是说专项 Tool 没有价值，而是问题还没有回答完：如果希望 Agent 真正处理这类业务，谁来组织它需要的知识、可用能力、当前状态、确认过程和完成标准？

它确实有点像做数学题：

> 一个问题 → 选择求解方法 → 组织事实与计算 → 给出结论

区别在于，人知道自己正在解题，模型并不知道。模型此刻在做什么，是由 Prompt、Skill、Context、Tools、审批和运行状态共同告诉它的。

一个原本只负责读稿的 Agent，即使突然拿到装备数据的读取和写入能力，也不会自然变成攻略 Agent。它看到的上下文、学到的方法和被检查的结果，仍然可能全部围绕“怎样读稿”。

那就把 Prompt 里的身份从“读稿”改成“做攻略”？

还不够。旧定义可能同时留在宿主提示词、Skill、Tool 说明、审批逻辑和事务状态里。只改其中一句，系统的其他部分仍然可能把模型拉回原来的工作方式。

问题于是从“改哪一句 Prompt”，变成了“谁来组织模型周围的整套工作条件”。这才是 Harness 真正进入工程讨论的地方。
...
说得通俗一点，模型提出下一步；Harness 为这一步准备工作条件，并处理它产生的后果。

Harness 最窄的说法只包括 System Prompt（系统提示词）和 Tool Loop（工具循环）；
...
从系统角度看，广义 Harness 可以理解为模型与外部环境之间的**运行支架**。它组织模型怎样获得信息、提出行动、接收反馈、延续状态，并把行动连接到真实执行。

为了避免一路发明新名词，下面只保留三个层次：**Harness 系统**指完整运行环境；**业务 Harness** 指其中一类完整领域问题的责任边界；**业务 Harness 版本**保存这类问题在某次发布中采用的方法与规则。
...
| --- | --- | --- | --- |
| Typed Tool（类型化工具） | 一个动作的输入输出结构与错误语义；前置条件、权限和副作用由执行层检查 | 不定义完整业务路径 | 读取当前装备、应用配装方案 |
| Workflow（工作流） | 预先编码的控制关系，包括顺序、条件分支、循环、并行、重试和结束条件 | 规定全部或部分控制路径 | 用户确认后应用方案，再重新读取 |
...
在本项目的责任划分中，跨 Tool 的领域求解方法由业务 Harness 版本拥有；需要确定性保证的控制关系，则由 Workflow、运行时检查或 Tool 自身的执行代码落实。
...
| 加载方式 | 可被检索、显式加载或由宿主要求读取 | 由运行时为一次具体业务选择并固定版本 |
| 对模型的作用 | 提供知识和操作方法 | 同时规定目标、知识角色、工具策略、阶段策略和完成条件 |
| 状态职责 | 通常不是业务状态的权威来源 | 声明需要什么状态，由任务状态存储保存 |
...
一次配装同时依赖真实产品状态、攻略、本次业务状态和对话历史，它们并不来自同一个地方。

Context 不是一个单独的存储位置，而是“当前代码或模型在这一刻可以使用哪些信息”的统称。讨论 Context 时，必须先说明这些信息来自哪里、保存多久、由谁使用，以及模型能不能看到。

OpenAI Agents SDK 区分本地运行上下文（Local Run Context）与模型可见上下文（Model-visible Context）：前者供 Tools、Hooks（钩子）和应用代码使用，后者才会进入模型请求。对话历史可以由 Session（会话记忆）管理；一次配装进行到哪里，则仍要由项目自己的业务状态保存。[OpenAI Agents SDK：Context management](https://openai.github.io/openai-agents-python/context/)

<div class="article-diagram context-diagram" role="img" aria-label="事实、业务状态和会话怎样被运行时组装成本地上下文与模型可见上下文">
  ...
  <div class="context-flow">
    <div class="context-sources">
      ...
      <span><b>本次业务状态</b>目标、待确认方案、确认状态、Harness 版本</span>
  ...
  <div class="diagram-feedback"><b>Tool 结果</b><i>→</i><span>成为新的读取证据；有副作用时更新产品状态或本次业务状态</span><i>→</i><b>运行时重新组装</b></div>
</div>

以配装为例，当前装备必须来自真实产品状态，装备数值必须来自装备数据目录，策略建议可以来自带适用条件的攻略，计算结果必须来自公式引擎。业务 Harness 声明需要哪些信息；运行时负责选择和组装；事实源与任务状态存储负责保存。

信息被选出来以后，Runtime 还要决定哪些进入本次模型输入、哪些只留在代码侧。Prompt 只是模型可见内容的一部分；身份、权限和验证状态仍属于本地运行上下文。

## Harness 和 Prompts 的关系

Prompt 经常同时指可以独立编写和版本化的 **Prompt 模板**，以及某次调用最终发送给模型的完整输入。两者必须分开。

Prompt 模板可以定义目标、角色、概念、方法、示例和表达要求，也可以通过消息角色建立指令优先级。但这种模型侧权威不能替代代码侧权限：模型即使收到“不得越权”的高优先级指令，执行层仍要对真实 Tool 调用再做检查。

| 对象 | 是什么 | 生命周期 | 主要作用 | 能否直接阻止副作用 |
| --- | --- | --- | --- | --- |
| Prompt 模板 | 可复用、可版本化的内容模板 | 跨多次调用 | 保存角色、概念、方法、示例和表达要求 | 不能 |
| 本次模型输入 | Prompt、对话、可见 Tools、Tool 结果和必要证据 | 一次模型调用 | 为模型形成当前工作视图 | 不能 |
| 运行时硬边界 | 权限、审批、Tool 可见性、版本一致性、写入范围和后置检查 | 完整业务过程 | 在代码侧允许、拒绝、保存和验证 | 可以在覆盖范围内阻止 |

Prompt 模板会成为本次模型输入的一部分；对话、证据和可见 Tools 也会一起进入。运行时规则则留在代码侧，不因为模型“已经知道”就停止检查。

在本项目中，Prompt、Skill 和工具权限可以独立维护，但业务 Harness 版本必须记录这次发布实际采用的版本。一次任务固定一个 Harness 版本，也就固定了这次工作依赖的规则；若领域知识允许读取最新版本，待确认方案还要记录实际证据版本，并在确认前重新检查是否过期。
...
第一类是**求解方法**：通常先看什么、需要哪些知识、怎样比较候选、什么时候追问。业务 Harness 版本把它作为强参考交给模型。第二类是本文概括可执行约束的项目术语——**硬边界**：权限、审批、状态不变量、写入范围和后置验证。这些必须落实为 Workflow、运行时、Tool 或产品代码，而不能只写给模型看。

“强参考”不是随便看看。业务 Harness 版本可以要求模型交付结构明确的候选装备组合、待确认配装方案和证据；系统可以检查模型交出了什么，却不能据此证明模型内部采用了唯一的推理路径。

本项目的业务 Harness 还需要具备四个特性：

1. **面向完整业务问题**：它负责的是配装、排轴这类可以独立完成的用户目标，不是某个 Tool 或某个实体；
2. **由持久化状态承载**：目标、用户限制、证据、候选、当前阶段和 Harness 版本不能只存在于聊天记录；
3. **以真实结果结束**：模型说“已经完成”不是完成，产品状态满足后置条件才是完成；
4. **可以独立迭代**：修改配装方法，不需要同时重新发布选人、排轴、BUFF 和计算。

### 为什么不能强制？

本项目不把整套**问题求解方法**实现成唯一强制路径。方法中可观察且必须保证的部分，仍然会落实为 Workflow、运行时前置条件、结构化结果和完成检查；模型保留的是分析、追问、取证和重新规划的空间。
...
单次模型调用只会产生一次输出。要让一项业务跨越多次模型调用、Tool 执行、用户确认和状态变化，需要把这次具体工作保存下来，再由一段程序持续协调。

本文把这次可以暂停和恢复的具体工作称为**业务事务**，把持续协调它的应用层程序称为 **Harness Runtime（Harness 运行时）**。

业务 Harness 版本是一份声明；运行时负责解释这份声明，并让它在一次业务事务中真正执行。

OpenAI Agents SDK 提供模型与 Tool 之间的基础运行循环、对话记忆和审批恢复；项目自己的 Harness Runtime 还要叠加业务事务、版本固定、工具权限和领域完成验证。[OpenAI Agents SDK：Running agents](https://openai.github.io/openai-agents-python/running_agents/)
...
业务 Harness 版本声明完成条件，Runtime 在关键结束点重新读取真实状态，并据此判断是否完成。运行时还要留下**执行轨迹（Trace）**，记录 Harness 版本及其引用的 Prompt、Skill 和工具权限版本、当时的产品状态、Tool 调用、用户确认，以及完成检查使用的证据与结论。Trace 不负责判定完成，它负责保存判定过程。
...
| Tools | 强调跨任务复用的通用能力 | 强调领域数据结构、语义写入范围和业务前置条件 |
...
几件装备放在一起，只是**装备组合**；当它绑定目标角色、用户限制、选择理由，以及形成方案时读取到的当前配装，才成为一份可以交给用户确认的**待确认配装方案**。这份方案不是一段聊天文字，而是需要持久化的业务对象：它记录产品状态快照和实际证据版本，供确认前判断方案是否已经过期。完整的配装业务要处理它的形成、确认、应用和验证。
...
| 怎样判断系统是否做对 | 完成条件、合同测试与真实界面（UI）验证；Trace 保存证据和结论 | 运行时教学 |

Tool 的输入、输出或错误约定发生不兼容变化时，通常还要发布新版本，并更新业务 Harness 版本中的引用，或者提供兼容层。

Prompt 当然仍然会独立迭代。需要改变的是：不再把所有业务规则都塞进一份全局 Prompt。运行时会结合当前 Harness 版本、事务状态、可见 Tools、对话历史和事实证据，组装本轮模型输入。
...
| 记录 | 稳定性 | 负责什么 | 例子 |
| --- | --- | --- | --- |
| 业务定义 | 相对稳定 | 稳定业务身份、完整责任、支持动作类型，以及最大的 Tool 与写入边界 | 配装可以改装备，不能改队伍成员 |
| 业务 Harness 版本 | 单个版本不可变；通过发布新版本持续迭代 | 具体求解方法、阶段、知识入口、允许使用的 Tools、异常处理、完成验证规则和表达要求 | 配装 V1、V2 |
| 业务事务实例 | 每次工作独立产生 | 目标、限制、当前阶段、证据、待确认方案、审批、产品版本和固定的 Harness 版本 | 正在为当前角色准备一套配装 |
...
单个业务 Harness 版本还必须固定它引用的 Prompt、Skill 和工具权限版本；业务事务固定整个 Harness 版本，才不会在运行途中悄悄换掉其中一项规则。

这里需要特别强调：业务事务不是数据库事务，副作用不会自动回滚；重试、补偿和重新规划都必须显式定义。
...
五个业务 Harness 共享工具注册、执行基础、知识来源和同一份产品状态；每个版本只开放自己允许使用的 Tools，因此共享能力不等于共享权限。
...
<div class="article-diagram harness-ecosystem-diagram" role="img" aria-label="管理器把跨业务请求拆成五个独立业务 Harness，它们共享工具、知识和产品状态，但各自拥有写入范围与完成验证">
  ...
  <div class="ecosystem-shared"><small>共同基础</small><span>工具注册与执行基础</span><span>游戏知识与公式</span><span>同一份产品状态</span><span>执行轨迹</span></div>
  <div class="ecosystem-stop"><b>前一步失败、被拒绝或过期</b><span>依赖该结果的后续任务不得继续使用旧方案</span></div>
</div>

这条箭头链只是一种请求形成的依赖图，其他请求可以得到不同的依赖关系。它不是五份更长的 Prompt，而是五个面向领域问题求解、可以独立实例化、版本化和验证的业务 Harness，以及管理它们持续演进的体系。
...
| 系统说完成，但真实页面没有变化 | 完成条件与结果验证；Trace 用于定位和举证 |
