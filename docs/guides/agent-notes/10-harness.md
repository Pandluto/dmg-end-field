...
Tool 只负责自己的动作合同。跨 Tool 的推荐方法属于业务 Harness，必须固定的顺序才属于 Workflow。
...
运行时还要留下**执行轨迹（Execution Trace，后文简称 Trace）**，记录本次使用的 Harness 版本、当时的产品状态、Tool 调用、用户确认、状态变化和完成证据。要判断“修改前是否真的等待了确认”，不能只看 Prompt 里有没有这句话，而要检查确认、执行和结果验证是否真的发生。
