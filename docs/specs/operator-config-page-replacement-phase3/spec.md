...
#### Scenario: display 层展开全局加成

- WHEN `panel.display` 从 `panel.calc` 派生显示字段
- THEN `panel.calc` 中的原子字段保持不拆分
- AND `allSkillDmgBonus / magicDmgBonus / allDmgBonus` 等字段在 `panel.calc` 中独立保存
- AND `panel.display` 负责将这些字段展开到对应显示项
- AND 展开结果仅用于展示，不反向参与计算
- AND `panel.calc` 消费 `operator / weapon / equipment` 时不执行这些显示拆分
...
- AND `magicDmgBonus` 展开到 `灼热 / 电磁 / 寒冷 / 自然` 伤害显示项，不展开到 `物理伤害加成`
...
- AND `ConfigSnapshot.panel.calc` SHALL 消费该好感值
...
- AND `panel.display` 只消费 `panel.calc` 派生后的结果
...
- AND `ConfigSnapshot.panel` 只消费 `equipment.totals`
- AND `ConfigSnapshot.panel` 不直接遍历 4 件装备明细
...
- AND `panel.display` 将 `magicDmgBonus` 展开到对应元素伤害显示项
...
- THEN 先取当前等级角色基础能力值
- AND 加上武器无条件能力值贡献
- AND 装备的 `mainStatBoost / subStatBoost` 只在主副属性百分比阶段参与计算
- AND Phase 3 不读取装备 `fixedStat` 或旧 `equipment` 手填对象来补固定能力值

#### Scenario: 主副属性

- WHEN 计算主副属性
- THEN 主属性字段来自角色模板 `mainStat`
...
- WHEN 计算最终主副属性
- THEN 先按旧 `OperatorConfigPanel` 公式记录，待复核纠正
...
- AND `mainStatScale` SHALL 包含装备 `equipment.totals.mainStatBoost`
- AND `subStatScale` SHALL 包含装备 `equipment.totals.subStatBoost`
...
- AND `panel.calc.abilityBonus` 内部单位暂按小数记录
- AND 旧链路兼容字段 `abilityBonus` 若继续暴露给伤害计算 SHALL 保持旧单位：百分数点，即 `panel.calc.abilityBonus * 100`

#### Scenario: 攻击力

- WHEN 计算面板攻击力
- THEN 先按旧 `OperatorConfigPanel` 的分步攻击力公式记录，待复核纠正
- AND `rawAtk = operatorAtk + weaponAtk`
- AND `equipmentAtkPercent = equipment.totals.atkPercentBoost`
- AND `atkPercent = weaponPassiveAtkPercent + equipmentAtkPercent`
- AND `baseAtk = rawAtk * (1 + atkPercent) + weaponPassiveAtk`
- AND `panelAtk = baseAtk * (1 + abilityBonus)`
- AND 攻击力详情 SHALL 展示 `基础攻击 / 百分比加成 / 最终基础 / 面板攻击` 四段
- AND `panel.calc.atkPercent` 内部单位暂按小数记录
- AND 旧链路兼容字段 `weaponAtkPercent` 若继续暴露给伤害计算 SHALL 保持旧语义：武器攻击百分比与装备攻击百分比之和
- AND 旧链路兼容字段 `weaponAtkPercent` 若继续暴露给伤害计算 SHALL 保持旧单位：百分数点，即 `panel.calc.atkPercent * 100`
- AND Phase 3 不读取 `equipment.flatAtk`

#### Scenario: 生命值

- WHEN 计算面板生命值
- THEN `panelHp = operatorHp * (1 + weaponHpPercent + equipment.totals.hpPercent)`
...
- AND `panel.calc.critDmg = 0.5 + equipment.totals.critDmgBonusBoost + weapon.totals.critDmgBonusBoost`
...
flowchart TD
  ...
  Calc --> AbilityChain["主副能力与 abilityBonus 链路"]
  AbilityChain --> AbilityRaw["rawMain/rawSub\n= operator 四维 + weapon 固定四维/主副固定 + favorValue"]
  AbilityRaw --> AbilityScale["main/sub/all scale\n= equipment 百分比 + 待复核 weapon 百分比"]
  ...
  Calc --> AtkChain["攻击力链路"]
  AtkChain --> RawAtk["rawAtk = operatorAtk + weaponAtk"]
  RawAtk --> AtkPercent["atkPercent = weaponAtkPercent + equipmentAtkPercent"]
  AtkPercent --> BaseAtk["baseAtk = rawAtk*(1+atkPercent)+weaponPassiveAtk"]
  BaseAtk --> PanelAtk["atk = baseAtk*(1+abilityBonus)"]

  Calc --> HpChain["生命值链路"]
  HpChain --> PanelHp["hp = operatorHp*(1 + weaponHpPercent + equipmentHpPercent)"]

  Calc --> OtherCalc["其他 calc 原子字段"]
  OtherCalc --> Crit["critRate / critDmg"]
  OtherCalc --> SourceSkill["sourceSkillBoost"]
  OtherCalc --> DamageBonus["damageBonus\n元素/技能/全局/特殊伤害"]

  AbilityBonus --> PanelCalc["ConfigSnapshot.panel.calc"]
  PanelAtk --> PanelCalc
  PanelHp --> PanelCalc
  Crit --> PanelCalc
  SourceSkill --> PanelCalc
  DamageBonus --> PanelCalc

  PanelCalc --> Compat["旧链路兼容字段"]
  Compat --> CompatAtk["weaponAtkPercent = atkPercent * 100"]
  Compat --> CompatAbility["abilityBonus = abilityBonus * 100"]

  PanelCalc --> Display["panel.display\n只消费 panel.calc"]
  Display --> Expand["展开 allSkillDmgBonus / magicDmgBonus / allDmgBonus"]
  Expand --> UI["常驻面板 + detailMarkdown"]
...
- WHEN 面板计算完成
- THEN `ConfigSnapshot.panel` 分为 `calc` 与 `display`
- AND `ConfigSnapshot.panel.calc` 保存最终计算结果、原子字段和必要中间值
...
- AND `panel.display` 只消费 `panel.calc`
- AND 页面常驻面板和 `detailMarkdown` 不直接从 `operator / weapon / equipment` 拼显示值
- AND `panel.calc` 至少包含 `atk / hp / strength / agility / intelligence / will / baseAtk / abilityBonus / operatorAtk / weaponAtk / weaponAtkPercent / critRate / critDmg / sourceSkill`
