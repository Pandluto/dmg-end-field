...
主界面 def-opencode 面板遇到高风险自然语言请求时，SHALL 优先创建 appdata/localdata AI work node 基线，而不是先写用户排轴快照。

高风险请求包括换人、清空排轴、批量大改动、批量删除/替换、用户明确要求“可回退”的操作。入口流程：

```text
用户请求
  -> createAiTimelineWorkNodeFromCurrent
  -> 取得 nodeId / saveId / branchId
  -> 将 nodeId 注入本轮 agent prompt
  -> agent 在需要时使用 diffAiTimelineWorkNode / checkoutAiTimelineWorkNode / restoreAiTimelineWorkNodeBase
```

用户 timeline snapshot 只能作为兼容回退和人工恢复资产，不应作为 AI 分支日志、修改日志或工作节点。AI 面板如果提供“回退到这条消息前”的入口，拥有 work node id 时 MUST 优先使用 `restoreAiTimelineWorkNodeBase`；只有没有 work node id 的旧消息才 MAY 回退到用户 timeline snapshot。
...
- renderer apply 与 appdata 回写暂时不是事务；如果回写失败，命令必须返回 `rollbackMarkError`，后续需要补偿重试入口。
- appdata API 已有基础结构校验；更深的按钮表、buff 引用一致性仍应以后续共享 validator 收敛，真正应用前必须以 renderer 校验为准。
