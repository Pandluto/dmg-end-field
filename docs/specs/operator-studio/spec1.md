...
1. 用户先选中某个技能和该技能下的一个 hit。
2. 用户点击“复制 Hit”。
3. 页面基于当前 hit 生成一个新的 hit key。
4. 页面复制 hit 名称、伤害属性、技能乘区和 `L1-L9 / M1-M3` 倍率矩阵。
5. 页面同步该技能的 `hitCount`。
6. 页面将新复制的 hit 设为当前选中项，供用户继续编辑。
...
- 本 spec 描述官方参考导入、本地库导入、保存、另存为、删除、复制 hit、导出和分享导入流程。
