...
4. Worktree-first 是复杂排轴重排、分支草稿、预览和需要版本证据的默认路径：先写 `workingPayload`，再按明确的应用策略更新当前排轴。
...
Phase 5 不得推翻已经稳定的 Phase 4 运行时闭环。以下能力是迁移期间必须保持的行为契约：

- 明确、低风险的单步自然话术继续可以直达既有受控工具；复杂、多步、重排、分支或用户要求先看结果的操作必须进入 Work Node 草稿路径。
- `patch_and_validate` 后的实时 checkout、完整 `skillButtonTable` 到 `staffLines` 的重建、整组 `copyStaffLine` 复制与 staff index 校验必须保持可用；不能为了新 Repository 再退回逐按钮拼接或刷新页面后才一致的旧行为。
- AI 模式下暂停手动拖拽、聊天记录恢复、会话作用域隔离、工具缓存刷新和失败结果可见化均属于既有可靠性保障，不得在重构时删除或绕过。
- 运行时“当前工作副本”与 Repository 中的 `CheckoutRef` 是两个层次：前者负责即时画布反馈，后者负责持久化的已应用事实。二者必须在一次成功确认中对齐，失败时都不得伪报成功。

因此，Phase 5 的目标不是把所有 AI 指令强行改成“先关闭再应用”，而是让每一种执行策略都有唯一的数据归属、可验证结果与清晰的 UI 语义。
...
用户自然话术
→ 按风险与意图分流
├─ 明确、低风险单步操作：既有受控工具 → 实时 checkout 回执 → 当前工作副本与 CheckoutRef 对齐
└─ 复杂/重排/分支/预览操作：创建或复用 Work Node
   → 受控 Patch 写入 workingPayload
   → validate + diff + risk
   → 按用户确认或既有 auto-low-risk 策略 checkout
   → CheckoutRef(work-node) + 当前工作副本更新 + 审计事件
```
