...
2026-07-07 手测中，连续定位、上下文追问、单按钮加 Buff、批量删除 Buff 的主流程效果已达到阶段验收标准，可以暂时放行继续推进底层架构。

本轮不把前端展示问题并入底层 agent kernel 修复，但后续需要专项处理：

- AI 面板尚未统一接入全局 markdown 渲染库，回复里的表格、列表、粗体、代码块和长段落展示仍不稳定。
- 工具调用和 DSML 片段仍可能直接暴露给用户，批量命令时尤其影响可读性。
- 长输出被截断时缺少“摘要 + 展开/查看原始日志”的分层展示。
- 批量命令只有松散的 pending/done 状态，缺少批次级进度、失败点、已完成摘要和剩余命令展示。
- 高风险批量操作的“用户快照回退点”和 appdata AI work node 回退必须在 UI 文案上明确区分，避免把 localStorage/sessionStorage 当前迁出态、用户排轴快照、appdata work node 混成同一种东西。
- work node diff / checkoutDecision 暂未形成可视化审核面板；短期通过命令结果和日志验收，后续应作为前端专项。
...
命令入口 MAY 兼容模型常见别名字段，但必须在 schema/执行前归一化为 canonical 字段。例如 `removeBuff.buffDisplayName` 可以归一化为 `displayName`。系统 SHALL NOT 在别名字段未识别或 Buff 条件未匹配时静默 fallback 到第一个 Buff。

`removeBuff` 特别要求：

- 删除指定 Buff 时必须提供 `buffId`、`displayName`、`name` 或受支持的别名字段。
- 删除按钮上的全部 Buff 必须显式提供 `all:true`。
- 指定 Buff 条件未匹配时命令必须失败，不得删除其他 Buff。
...
当 evidence 中包含 `batchId` 时，verifier SHALL 以批次为单位给出更具体的失败或等待信息，例如批次总数、done/error/pending/running 数量和失败命令。批次级 verifier 不得把 enqueue 成功视为业务完成。
