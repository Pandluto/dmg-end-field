...
- `npm run test:def-native-catalog`
  ...
  - session artifact 通过临时目录+rename 落盘，manifest hash、TTL reuse/cleanup 和 `../` 文件名拒绝均通过；
  - adapter 合同确认 `retrieval/**` 只有 read allow，edit 仍仅允许 `node/working/**`，并保留 `external_directory: deny`。
...
- `grep`/`glob` 的 upstream permission 只按 pattern 判定；其路径仍经 OpenCode session-root `external_directory` guard，DEF 保持 `external_directory: deny`，不开放项目、raw storage、Share Data 或其他 session。
