...
Task 8-1-2 is **complete**. Package/Registry, native session pinning, real native Scenario execution, minimal regression, safety gate, promotion and rollback all have v1-backed evidence. Package self-checks remain explicitly non-Agent checks.
...
Focused boundary checks also passed:

- Snapshot-unavailable run `native-harness-run-08137f76-4591-4b67-8cb8-59cda6914914` observed v1 `snapshotAvailable=false` and returned `BLOCKED_ENVIRONMENT` / `snapshot-unavailable` before creating a fixture or sending a turn (`turnCount=0`). The temporary service interruption was restored immediately; v1 subsequently reported `ready=true`, `snapshotAvailable=true`, and `uiConsumerCount=1`.
- `evaluatorOnlyInput` is retained only by the evaluator call frame, never passed to `runNativeScenario`, package construction, or written run data. The focused check verifies both that a normal public evaluator result contains no sentinel and that disclosure causes `ERROR_VERIFIER` instead of a passing regression.
...
- There is no hidden-regression platform or Harness-provided hidden data in 8-1-2; the implemented evaluator-only sentinel boundary is the deliberately small interface required for this task.
