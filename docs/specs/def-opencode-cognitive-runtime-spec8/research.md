...
不推荐把完整 `sources/yz-game-knowledge-bundle.md` 放入 system prompt，也不推荐现在引入多 Agent 群、通用向量记忆平台或模型微调。
...
3. `sources/yz-game-knowledge-bundle.md` 是 skill、glossary 与整理攻略的拼接包，不是带 URL、时间戳、版本、原句定位的不可变证据仓。
...
输入不再是单个 `sources/yz-game-knowledge-bundle.md`，而是一条来源的不可变记录：
...
- 不将约 80 KB 的 `sources/yz-game-knowledge-bundle.md` 每轮加载到 prompt；
