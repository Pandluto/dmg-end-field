...
- [ ] 固定 legacy endpoint 清单：AI CLI、四领域 current/library/template/check/apply、proposal list/show、Agent records/events/scripts。
- [ ] 固定 storage key、proposal Wait/Yes/No 两段状态、REST 禁止命令与 pending proposal 限制。
- [ ] 为四领域至少各保存一个 current/library/template/check/apply wire fixture；fixture 必须清除个人数据和绝对路径。
- [ ] 固定 `17321` DEF routes、tool registry、route map、tool schema hash、health 必需字段。
- [ ] 枚举 Electron、dev-agent、DEF sidecar、OpenCode plugin、renderer、smoke/contract script 对 `17321` 的调用。
- [ ] 明确记录当前 `/ai-cli` 是原生 OpenCode iframe；验证是否仍有任何真实 Host UI 能调用旧 `runAiCliCommand` approve/save。
- [ ] 记录外部工具目录 78 个文件的迁移分类、硬编码 REST 调用方和发布排除清单。
- [ ] 记录当前 now-storage ↔ browser sync 方向和 library 变化事件。
...
- [ ] 将 read/current/library 和 apply/save 从同一 adapter interface 拆为 pure domain adapter 与 storage/Host adapter。
- [ ] 提取 proposal type、base identity、review manifest 和 deterministic digest。
- [ ] schema/template 从同一 core 生成，删除新增手抄 schema 的可能性。
- [ ] 保留旧 localStorage writer adapter，使旧 REST fixture 行为不变。
- [ ] 纯 core import 在没有 `window` 的 Node 进程中成功。
- [ ] 将 prompt `?raw`、浏览器事件、sessionStorage 选择态留在 core 之外。
...
- [ ] 实现 spec 中 8 类 versioned resources。
- [ ] 实现 `fill_get_current`、`fill_search_library`、`fill_get_template`、`fill_validate`。
- [ ] 实现 `proposal_create`、`proposal_list`、`proposal_inspect`。
- [ ] 所有 tool 定义 input/output schema、structured errors、size/page limits。
- [ ] Streamable HTTP 使用单实例 proposal DB 和 Host snapshot。
- [ ] STDIO facade 只转发到 daemon；stdout 仅 MCP，日志到 stderr。
- [ ] loopback bind、Origin/Host validation、每 client auth、token redaction。
- [ ] transport session 与 owner namespace 分离。
- [ ] 文档明确不依赖 MCP Tasks、sampling、elicitation。
- [ ] SDK 版本锁定；v2 升级另开任务，不与首轮切流合并。
...
- [ ] UI 展示 target、base revision、normalized draft、逐字段 diff、validation、warnings、evidence、requested writes、digest。
- [ ] 只有真实产品 UI 事件可触发 approve/reject/save。
- [ ] 首版按产品决定保留两步审核/保存或合并 UI；底层仍分别记录 review/persistence。
- [ ] 用户动作绑定 proposal revision、manifest digest 与当前 library revision。
- [ ] stale 时禁止保存并提示重新生成/rebase，不让模型自动覆盖。
- [ ] Host writer 写入后重读，postcondition 失败标记 failed 并保留审计。
- [ ] 成功后发布 revision/change event，相关页面立即重读。
- [ ] 不再要求用户在聊天中键入 Y/Y；旧 proposal origin 在兼容窗口仍可识别。
...
- [ ] 将 `CLAUDE.md` 协议部分替换为由 core/MCP schema 生成的引用。
- [ ] 筛选填写策略，明确标注 strategy 而非 protocol。
- [ ] 将 `golden-examples.md` 拆为按 schema version 验证的 fixtures/resources。
- [ ] 将 AKEDatabase Skill 缩为 MCP 路由说明；若无额外价值，提交独立移除提案。
- [ ] 给 `common_http.py` 和硬编码 REST caller 提供 MCP 替代说明。
- [ ] 历史 Python/MJS/JSON 默认归档，不作为发布运行时。
- [ ] 发布 allowlist 排除缓存、临时请求、个人草稿、Windows 绝对路径、异常文件名。
- [ ] 完成调用方清单，标记 migrated/archived/blocked/owner。
...
- [ ] runtime 启动与预热；
- [ ] native session create/recover；
- [ ] Workbench timeline admission；
- [ ] session/axis binding；
- [ ] current checkout context；
- [ ] tool registry/route-map/schema parity；
- [ ] native typed tool read；
- [ ] 低风险 current mutation + command/result/postcondition；
- [ ] native permission + approval capability；
- [ ] Work Node fork/materialize/sync/validate/diff/use/restore（涉及核心/transport 阶段必须全做）；
- [ ] Timeline Repository/SQLite restart/lifecycle；
- [ ] Interop/Harness v1 events/transcript/questions/state；
- [ ] 真实 iframe UI 可见性；
- [ ] Required Record 完整，黑盒 prompt 不泄漏测试意图。
