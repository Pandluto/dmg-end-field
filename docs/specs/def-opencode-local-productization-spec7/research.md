...
> 其实我建议分为3大类工具；def-节点代码修改；def-节点简单增删改查；def-数据资源（武器干员装备填表这部分）。

后续设计必须严格保持这三类，不新增一个与它们并列或凌驾其上的“类写代码工具模型”分类。“类写代码”是第一类 `def-节点代码修改` 的执行方式：Workbench 中的模型像修改代码一样自由编辑节点，而不是增加一个名为“排轴”的专用 tool，也不是把旧按钮操作重新包装成更多 typed tools。

```text
复制当前节点形成隔离子节点
  → 把节点内容呈现为稳定、可读、可编辑的文件
  → 模型使用 OpenCode 原生 read/edit/apply_patch 自由修改
  → 从文件重建节点 payload
  → validate + diff
  → approval
  → 直接 use 子节点
...
这里的代码式修改可以跨字段、跨按钮、跨组、跨时间位置进行组合编辑，不要求每一种业务动作都预先存在一把专用工具。它是 `def-节点代码修改` 的核心能力，而不是第四类工具。

三类工具的分工必须继续保持：

- `def-节点代码修改`（注册 id：`def-node-code`）：提供节点文件、OpenCode 原生 read/edit/apply_patch、payload 重建和校验边界，承载自由排轴与组合修改；
- `def-节点简单增删改查`（注册 id：`def-node-crud`）：负责 fork/bind/list/create/update/delete/validate/diff/approval/use/restore 等明确的节点生命周期操作，不取代自由编辑；
- `def-数据资源`（注册 id：`def-data-resource`）：负责武器、干员、装备、技能、Buff、伤害和填表所需的可信数据，供前两类工具使用，不直接替模型编造数据。

因此后续实现必须避免三种倒退：

1. 重新为“移动按钮”“复制技能”“添加 Buff”“调整时间”等每个动作建立长链专用工具，并迫使模型只能组合这些按钮级命令；
2. 把 Workbench 退化为查数据后让用户手动排轴的咨询助手；
3. 把 OpenCode 原生编辑能力藏起来，只在 prompt 中声称支持自由修改，实际仍走旧 REST command/JSON/DSL。

前端也必须围绕该主体设计。原生文件编辑、tool call、节点 diff、validation、approval 和 use 是一条连续工作流；“节点变更”不是单纯展示 Git diff，而是代码式节点编辑的审查面。用户不必理解 `working-payload.json` 的技术细节，但必须能看到模型创建了哪个子节点、修改了什么、校验是否通过、是否已应用。

第一类 `def-节点代码修改` 在两个宿主中的权重不同：

- Workbench AI mode：默认并优先使用，面向当前主界面进行自由排轴和节点修改；
- `/AI CLI`：职责仍以数据资源、填表、资料处理和独立 Agent 工作为主，仅在它自己的任务明确创建/绑定节点时使用，不继承 Workbench 当前节点或会话。

三类工具的边界高于视觉裁剪和入口调整。若最终界面更像 DEF，但 Workbench 的第一类工具仍不能自由修改节点，或数据资源与节点 CRUD 再次混成一套按钮级命令，则 Spec 7 仍然失败。

第一类工具的当前实现缺口、推荐代码工作区、双向 codec、权限矩阵、校验/diff/risk、失败恢复和分阶段落地见 [`research-def-node-code.md`](./research-def-node-code.md)。该专项研究是后续 Spec 7 定稿的组成部分，不能再用一句“允许 read/edit/apply_patch”替代。

## 三、Spec 6 新暴露的遗留缺陷

以下内容不是 Spec 7 的新增范围，而是 Spec 6 已规定但真实使用后发现尚未兑现的内容。应在进入视觉本地化前先修复，避免用新设计掩盖职责串线。

### 3.1 Workbench agent 身份串线
...
Workbench 不需要一把名为“排轴”的专用 tool；排轴应当以 `def-node-code` 类写代码修改为主体，并组合 `def-node-crud` 与 `def-data-resource`。Prompt 与 Workbench skill 必须明确把自然语言映射为节点流程：
...
### 3.4 历史恢复只恢复了 OpenCode session，没有恢复 DEF 工作历史
