...
本项目反复确认的核心不是增加一个名为“排轴”的专用 tool，也不是把旧按钮操作重新包装成更多 typed tools。Workbench AI mode 的主要执行方式必须是“类写代码工具模型”：
...
这里的“类写代码”指模型像修改代码一样修改节点内容：可以跨字段、跨按钮、跨组、跨时间位置进行组合编辑，不要求每一种业务动作都预先存在一把专用工具。它是 Workbench 的主体能力，而不是隐藏在工具清单中的一个小类。

三类工具的分工必须继续保持：

- `def-node-code`：主体执行模式。提供节点文件、原生读取与编辑、重建和校验边界，承载自由排轴与组合修改；
- `def-node-crud`：节点生命周期与简单明确操作，负责 fork/bind/list/delete/validate/diff/approval/use/restore，不取代自由编辑；
- `def-data-resource`：提供可信干员、武器、装备、技能、Buff 和伤害数据，供代码式节点修改引用，不直接替模型编造节点内容。
...
这项原则高于视觉裁剪和入口调整。若最终界面更像 DEF，但模型仍只能查数据或调用按钮级工具，则 Spec 7 仍然失败。
