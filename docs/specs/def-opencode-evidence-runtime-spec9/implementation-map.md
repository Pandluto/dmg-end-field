...
| `agent/harness/scenarios/equipment-3plus1-unresolved-v1.json` | 真实 3+1 unresolved Scenario v3 | 使用 G2 的“寒冷伤害是否触发潮涌第二段”问题；校验 typed contract/state 与最后可见回答的完整结论 |
...
unresolved v3。四类都采用每轮精确 Tool allowlist；unresolved 还验证 typed contract/state
和最后可见回答的完整受限结论。
