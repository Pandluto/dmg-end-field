...
| Operator Configuration preview/patch | renderer 的本地产品库 | 与 active catalog 分离的写入边界；preview 必须再次精确解析产品，缺失时以 `operator-config-*-library-unavailable` 阻断，不得以名称或近似本地项替换 |
