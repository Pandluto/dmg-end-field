...
| `agent/harness/scenarios/equipment-3plus1-unresolved-v1.json` | 真实 3+1 unresolved Scenario | W6 使用 G2 的“寒冷伤害是否触发潮涌第二段”证据不足问题；必须同时证明 typed `UNRESOLVED` 与最终“不能证明”说明 |
...
| `npm run test:def-harness-scenario-verification` | PASS | turn/order/旧链禁止/typed state/最终回答验证通过；`UNRESOLVED` 不可仅靠 trace 或文案冒充 |
...
四类 W6 语义场景的静态合同已冻结：指定套装、未指定套装、两轮 correction、
以及“寒冷伤害是否触发潮涌第二段”的 unresolved。新的 unresolved Scenario 已提升到
version 2；它的旧 version 1 package-check 不再作为当前合同证据。
topology 与 set-selection 保留已核实的 package-check；correction 的两轮 trace 和
unresolved v2 的 typed/final outcome 由 scenario verification contract 覆盖。当前 worktree
没有候选 runtime registry，因而不伪造新的 package-check artifact。
首个真实 topology run 在调用任何 Tool 前被 provider 401 阻断，并已完成 runner cleanup。
其余三类真实会话、candidate regression 和 G5 相邻只读回归均为
`NOT_RUN_AFTER_PROVIDER_BLOCK`；不把它们归因于候选行为，也不以静态合同替代运行结论。
