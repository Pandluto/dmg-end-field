...
`BLOCKED_EXTERNAL_PROVIDER_AUTH_AND_CURRENT_RUNTIME_EVIDENCE`
...
| W6 final hardening | `9bd6c76` 至 `8ee48c5` | renderer cleanup 授权与错误语义、完整 3+1 候选优先、F1 Service matrix、Scenario typed/final-visible/反向断言合同全部收口 |
| W6 final runtime | `9e0b6240e1284a7ed574c89ac379ac42e2d99a26` | 含全部最终代码提交的隔离运行时；AgentRelease `eaf599b5b6324da74515308d3c5fa080d5a693d0b27b046085429d60eb944bc7` |
| Post-W6 candidate/session hardening | `01a3044` 至 `e8725cd` | 收紧模型可见 Schema、typed envelope、turn termination、Harness activation/seal 与 managed Session directory；只有自动合同，尚无新的真实 candidate runtime 证据 |
...
Immutable Harness ref 没有因 Tool/Service/Host 源码变化而自动变更，但“Harness package hash 未变”不等于当前组件组合已经在真实 AgentRelease 中运行。本目标树内最后可追溯的真实 candidate 证据仍是 W6 runtime `9e0b624` / release `eaf599…`；它只能作为历史证据。
...
最终运行态同时为 `uiConnected=true`；Workbench 当前可见文案是 `排轴助手` 与
`描述你想查看、调整或应用的排轴…`，Windows 测试文档已删除旧文案门槛。
...
Agent
  → def_data_equipment_3plus1_recommend
      requirements? = [{ operator-element-damage-triggers-set-effect, secondary }]
...
可选 requirement 只声明要验证的受控关系；伤害类型和证据由 Service 从可信 operator/set facts 派生，Agent 不能提供。
...
| `agent/harness/scenarios/equipment-3plus1-unresolved-v1.json` | 真实 3+1 unresolved Scenario v6 | 使用 G2 的“寒冷伤害是否触发潮涌第二段”问题；最后可见回答排除 ignored text，并按 Unicode 标点类别切分后验证结构化分句规则 |
...
| 当前位置 | 新位置/导出 | 迁移策略 |
| --- | --- | --- |
| `readDefEquipmentLibrarySource` | REST legacy adapters | 保留给旧 catalog/primitive 路径；不再是新 recommend 的产品数据源 |
| Data Management `readActiveGameCatalog` | `equipment-3plus1-active-catalog-reader.mjs` | 新 recommend 一次捕获同版 operator 与 equipment-set payload |
| `def.weapon.resolve` / `def.equipment.resolve` | Data Management active Catalog v2 | 通用 typed 资源查询也读取不可变 active catalog，并返回 `sourceRef`、`dataVersion` 与 catalog hash；active pointer、manifest 或 hash 无效时 fail closed，不回退 localStorage |
| Operator Configuration preview/patch | renderer 的本地产品库 | 与 active catalog 分离的写入边界；preview/patch 必须再次按稳定 ID 精确解析产品。武器还必须校验该 ID 与名称属于同一项；缺失、仅名称或同名不同 ID 时阻断，不得以近似本地项替换 |
| Catalog v1 扁平 `equipments` | Catalog v2 `equipment_sets` | 新表保存完整 gear-set payload，不从装备行反向猜 set bonus |
...
Active reader 为一次 Service composition 缓存一个 `readActiveGameCatalog()` 结果，对内只返回 `operators`、`equipmentLibrary.gearSets` 与 `catalog:<dataVersion>`，不暴露 SQLite 路径。存在 active pointer 时，无效指针、缺失 manifest、schema 不是 v2、catalog hash 或 payload hash 不匹配均 fail closed，不得静默回退 builtin；只有没有 active pointer 时使用 builtin Catalog v2。
...
| 原无受控 trigger 关系验证 | `evaluateDefEquipmentThreePlusOneRequirement` | 只由 composite Service 在所选套装上调用；不给 legacy route 增加语义 |

旧 route 可以保留 token、capability、artifact 与 revision adapter。
它们不得保留第二份 topology、ranking 或 solver。

Requirement evaluator 只读 operator `element` 和所选套装 effect 的 `stage/trigger`。唯一 `stage='secondary'` 且 trigger 完整声明 `damage-count/equipper/damageType/positive count` 并与 operator damage type 匹配时才产生 PROVEN evidence；缺失、重复、不完整或不匹配都是 UNRESOLVED。`name/label/condition/description/raw` 不进入证明决策。
...
源码盘点基线为 `0a01c19`。
下表是当时只有文档提交的开工历史基线，不是当前 integration `3102aaa` 的测试结论。
...
| `npm run test:def-harness-scenario-verification` | PASS | 每轮精确 Tool allowlist、typed contract/state、最后可见回答与相反断言验证通过 |
...
Computer Use 已在先前 W6 运行时验证 DEF Shell cleanup 按钮、显式保留选择、确认与取消路径。
最终 W6 renderer 再次验证按钮仅在 Agent 页、keep 必选和无 renderer capability 时失败闭合；
原生窗口重演被 Windows 前台锁阻塞，没有绕过确认或执行真实删除。
Chrome 与 Interop 已验证真实 Workbench、AI 模式、快照和候选绑定。

四类 W6 语义场景的当前版本为 topology v2、set-selection v2、correction v2、
unresolved v6。四类都采用每轮精确 Tool allowlist；unresolved 还验证 typed contract/state
和最后可见回答的完整受限结论。回答按 Unicode `Po` / `Pd` / `Pc` 与换行切成分句，
再由 `allOf` / `anyOf` / `noneOf` 判断同一分句中的关系，不维护逐个标点正则。

受控 requirement 合入后，unresolved v6 的真实验收还必须从 Tool trace 证明 call input 含有 `[{ kind: "operator-element-damage-triggers-set-effect", setEffect: "secondary" }]`。静态 Scenario verifier 对 typed UNRESOLVED 和最终分句的证明不能代替这个真实输入证据；省略 requirement 的普通 V1 调用仍应 READY。
...
当前 worktree 没有候选 runtime registry，因而不伪造当前版本的 package-check。
历史 topology v1 run 在调用任何 Tool 前被 provider 401 阻断，并已完成 runner cleanup；
四类当前版本真实会话、candidate regression 和 G5 相邻只读回归均为
`NOT_RUN_AFTER_PROVIDER_BLOCK`。不把静态合同或历史 run 写成当前版本的黑盒 PASS。

此外，当前 integration 晚于最后真实 W6 AgentRelease。即使 provider 恢复，也必须先从当前 HEAD 重建 runtime、记录新 AgentRelease 并绑定 fresh Session，再运行四类场景、candidate regression 和 G5；不得直接复用 `9e0b624` / `eaf599…` 作当前组件组合的 PASS。
