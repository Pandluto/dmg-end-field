...
实施代码已推进到 integration `3102aaa`；外部 provider 仍阻塞，不可 activation。

W1–W6 之后又合入了 candidate/session 运行边界加固、Catalog v2 active reader 和受控 trigger requirement。这些是当前源码与自动合同事实，不是新的真实 candidate 会话 PASS。本目标树内最后可追溯的真实 candidate runtime 仍是 `9e0b6240e1284a7ed574c89ac379ac42e2d99a26`，AgentRelease 为 `eaf599b5b6324da74515308d3c5fa080d5a693d0b27b046085429d60eb944bc7`；它早于上述后续实现。

候选仍未 promotion，stable pointer 保持不变；真实 provider 场景恢复前，
还必须用当前 HEAD 重建 runtime、记录新 AgentRelease、绑定 fresh Session 并重跑黑盒；不得把当前状态写成 runtime PASS 或 activation-ready。
...
- `requirements` 只声明需要 Service 验证的受控关系，不声明结论；
- 用户询问干员元素伤害是否触发所选套装第二段时，Agent 必须提交上述精确枚举；
- Agent 不得在 requirement 中携带 `proof`、`damageType`、证据文本或其他字段；
- 纠正请求必须提交完整的新输入；
...
| `requirements` | 0–1 项；只允许固定 `kind` + `setEffect=secondary` | `[]` |
...
Root、constraints、compare item 和 requirement item 都使用 `additionalProperties=false`。
...
type DefEquipment3Plus1RecommendationV1 = {
  ...
  requirementEvidence?: Array<{
    ...
    damageType: 'ice' | 'fire' | 'electric' | 'nature' | 'physical'
    setEffectId: string
    factPath: string
    trigger: {
      kind: 'damage-count'
      producer: 'equipper'
      damageType: 'ice' | 'fire' | 'electric' | 'nature' | 'physical' | 'magic'
...
- 提交受控 requirement 时，只有完整 typed trigger 与干员伤害类型精确一致才能在 READY 结果中返回 0–1 项 `requirementEvidence.state=PROVEN`；
- 无法唯一确定 secondary effect、trigger 事实不完整或 damage type 不匹配时返回 `UNRESOLVED`；
- 省略 `requirements` 时不返回 `requirementEvidence`，保持原 V1 结果形状和普通推荐行为；
...
- 非空时的规范化 `requirements`；
- 不包含 `priorPlanDigest`。

为保持 V1 兼容，`requirements` 省略或为空时不向 request identity 增加新 key，该调用的 `requestDigest` 必须与扩展前字节一致。
...
5. 指定套装解析或未指定套装筛选；
6. 可选受控 requirement 的 typed trigger 可达性验证；
7. 四个物理槽位与至少三件套装归属；
8. catalog 允许时的重复配件；
9. 四件同套合法与散件严格改善条件；
10. 有界候选、排序依据、缺失与歧义；
11. correction 的全量重算与旧 plan 失效关系；
12. 结构化终态和失败出口。

所有阶段消费同一个 catalog snapshot。
内部 helper 可以复用现有实现，但不能复制算法。

受控 requirement 的证明只能消费可信 operator catalog 的 `element` 和所选套装效果的结构化事实。目标 effect 必须唯一且 `stage='secondary'`，trigger 必须完整声明 `kind='damage-count'`、`producer='equipper'`、受控 `damageType` 与正整数 `count`，并与干员元素伤害类型一致。`name`、`label`、`condition`、`description` 和 `raw` 只能用于展示，不能参与证明决策。
...
实现固定为两个领域模块、一个 active catalog reader 和两个共享工具：

| 模块 | 唯一职责 |
| --- | --- |
| `scripts/def-core/stable-json.mjs` | object key 排序、undefined 省略、稳定 JSON 与 SHA-256；供 equipment、weapon 和 Profile 共同复用 |
| `scripts/def-core/native-catalog-value.mjs` | catalog 文本规范化与只读业务值投影；供 equipment 与现有 weapon catalog 共同复用 |
| `scripts/def-core/equipment-3plus1-active-catalog-reader.mjs` | 从 Data Management 的当前 active/builtin Catalog v2 一次性捕获 operator 与完整 equipment-set 事实；不暴露 SQLite 路径 |
| `scripts/def-core/equipment-3plus1-domain.mjs` | catalog 投影、规范化、stable id 解析、套装筛选、typed trigger requirement、拓扑、重复策略、约束、排名与 Digest 纯函数 |
...
`native-catalog-value.mjs` 固定导出：

```ts
normalizeDefCatalogIdentity(value)
projectDefCatalogSafeValue(value)
```

它承接当前 REST 文件中 equipment 与 weapon 共用的
`nativeCatalogText()` 和 `nativeCatalogSafeBusinessValue()`。
Equipment Domain 与 REST 中保留的 weapon adapter 都必须导入它。
不得让 weapon 反向依赖 3+1 Domain，也不得复制安全投影逻辑。
Identity 规范化必须保持现有 NFKC、别名字符清理与套装后缀语义。
安全投影的 blocked key 集合和最大递归深度必须保持不变。
公开函数不接收可放宽深度或 blocked key 的参数。
...
产品 composition 中的 `readOperatorCatalog()` 和 `readEquipmentLibrarySource()` 必须由 `createDefEquipment3Plus1ActiveCatalogReaders()` 提供。该 adapter 在一次 Recommendation Service 组合中只调用一次 Data Management `readActiveGameCatalog()`，使 operator 和 equipment 来自同一个不可变 data version；对 Service 只暴露 `storageKey='catalog:<dataVersion>'` 和业务 payload，不暴露 `databasePath`。

Catalog v2 固定增加 `equipment_sets` 表，每行保留完整 gear-set payload，因为扁平 `equipments` 行无法重建套装 bonus 和其他 set metadata。存在 active pointer 时，指针、release manifest、Catalog schema v2、catalog hash 和行 payload hash 任一无效都必须 fail closed，不得静默回退到 builtin。仅在不存在 active pointer 时读取 builtin Catalog v2。
...
它不再教授内部阶段顺序。
它必须说明：当用户询问干员元素伤害是否触发所选套装 secondary effect 时，填入精确的受控 requirement；该项只声明验证目标，不得由模型提供 proof、damage type 或 prose。
...
现状盘点还必须把 immutable Harness package 视为真实运行时消费者。
只要受支持的 package 仍引用旧 Tool，就不得提前删除对应 native target 或 OpenCode export。

Spec 9 的候选实施阶段固定保留这三个 primitive 的模型入口。
它们只用于旧 Harness 与旧 Session 兼容，不再进入新 candidate 的教学和验收链路。
候选 Scenario 必须禁止调用它们。
...
旧入口的退出是 activation 之后的独立兼容性门。
只有满足以下条件才允许另开任务取消暴露：

- 新 candidate 已经人工 activation；
- 不再支持任何引用旧 Tool 的 Session/Harness，或已有明确迁移策略；
- Tool reference contract 能区分 active package 与历史 immutable package；
- 旧 Sidecar 合同仍有明确的保留或删除结论。

同一个 `AgentReleaseV1` 不得同时教授复合路径与旧模型编排路径。
历史 immutable package 可以保留旧文本，但不能被 candidate 加载。
...
Spec 9 的实现分支可以同时包含新 Tool、运行时教学清理和 candidate，
但在 W6 完成前不得作为正式默认运行时落地。
W6 只测试显式绑定新 candidate 的 fresh Session。
人工 activation 是单独的切换决定，不属于首轮并行编码。
...
- Catalog v2 保留完整 equipment-set payload，operator 与 equipment 由同一 active reader capture 读取；
- active pointer 损坏、active release 缺少 schema-v2 capability 或 payload hash 不匹配时 fail closed，不回退 builtin；
- 省略 trigger requirement 的普通别礼+潮涌调用保持 READY 与旧 request digest；
- 受控 requirement 在 secondary/trigger 缺失、damage type 不匹配或存在多个 secondary effect 时返回 UNRESOLVED；
- 完整且匹配的 structured trigger 返回 READY + PROVEN requirement evidence；
- requirement 中的额外 proof、damage type 或未知字段返回 400；
- 双配件合法；
- 四件同套合法；
- 用户禁止重复配件；
...
同一场景还必须在真实 Tool call input 中观察到：
...
仅看到 UNRESOLVED 终态和最终文案不能证明模型已进入受控验证合同；普通别礼+潮涌调用在省略该字段时仍应按 V1 返回 READY。
...
最后文本按 Unicode `Po` / `Pd` / `Pc` 标点类别和换行切成分句，括号及成对引号保留在分句内。
Scenario 只声明 `allOf` / `anyOf` / `noneOf` 子串关系，不再维护标点枚举正则。
...
- 3+1 只从 Catalog v2 的当前 active/builtin 可信数据读取干员与完整套装；
- 用户点名的元素伤害→套装第二段关系通过受控 requirement 与 typed facts 验证，不从 prose 猜测；
- 指定和未指定套装均不需要 Agent 编排原子 Tool；
- 显式绑定新 candidate 的 AgentRelease 所使用的 Base Prompt、Runtime Skill、Harness 与 Tool description 不再教授旧链；
- immutable stable package 与旧 primitive 的兼容保留、owner 和退出门已有记录；
