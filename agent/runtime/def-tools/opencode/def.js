...
import {
  ...
  DEF_EQUIPMENT_3PLUS1_QUERY_MAX_LENGTH,
...
export function beginDefToolTurnFromChatMessage(sessionID, turnID, parts = [], options = {}) {
  ...
  installDefToolTurnPolicy(sessionID, turnID, classifyDefExecutableTurnPolicy(userText, {
    equipment3Plus1Enabled: options.equipment3Plus1Enabled === true,
  }))
...
export function applyDefToolModelMessagePolicy(messages, phase = 'generation', options = {}) {
  if (phase !== 'generation' || !Array.isArray(messages)) return false
  const equipment3Plus1Enabled = options.equipment3Plus1Enabled === true
  ...
  if (!equipment3Plus1Enabled) return false
...
export function assertDefToolTurnNotBlocked(sessionID, toolName, args = {}, options = {}) {
  const equipment3Plus1Enabled = options.equipment3Plus1Enabled === true
  if (toolName === 'def_data_equipment_3plus1_recommend' && !equipment3Plus1Enabled) {
    const error = new Error('def-tool-turn-policy-blocked: def_data_equipment_3plus1_recommend was not attempted because this Session is not bound to the def-equipment-3plus1-composite Harness.')
    error.code = 'def-tool-turn-policy-blocked'
    error.details = {
      attempted: false,
      attemptedTool: toolName,
      policy: 'equipment-3plus1-harness-activation',
      nextAction: 'Use the Session-bound Harness behavior; ordinary text cannot activate the composite candidate.',
    }
    throw error
  }
  const turnId = activeDefToolTurns.get(sessionID)
  if (!turnId) {
    if (toolName === 'def_data_equipment_3plus1_recommend') {
      const error = new Error('def-tool-turn-policy-blocked: def_data_equipment_3plus1_recommend was not attempted because no current 3+1 turn policy was reconstructed.')
      error.code = 'def-tool-turn-policy-blocked'
      error.details = {
        attempted: false,
        attemptedTool: toolName,
        policy: 'equipment-3plus1-composite',
        nextAction: 'Start a fresh bound user turn so its policy can be reconstructed before calling the composite capability.',
      }
      throw error
    }
    return
  ...
  if (toolName === 'def_data_equipment_3plus1_recommend' && turnPolicy?.kind !== 'equipment-3plus1-composite') {
    const error = new Error('def-tool-turn-policy-blocked: def_data_equipment_3plus1_recommend was not attempted because the current bound turn is not a classified 3+1 recommendation or correction.')
    error.code = 'def-tool-turn-policy-blocked'
    error.details = {
      attempted: false,
      attemptedTool: toolName,
      policy: 'equipment-3plus1-composite',
      nextAction: 'Follow the current user turn instead of activating the composite capability from unrelated text.',
    }
    throw error
  }
...
const defEquipment3plus1NormalizedQuery = tool.schema.preprocess(
  normalizeDefEquipment3Plus1Query,
...
const defEquipment3plus1RecommendArgs = tool.schema.object(
  ...
  if (count > DEF_EQUIPMENT_3PLUS1_MAX_DISTINCT_CONSTRAINT_QUERIES) {
    context.addIssue({
      code: 'custom',
      path: ['constraints'],
      message: `requiredEquipmentQueries, excludedEquipmentQueries, and compareEquipmentQueries may contain at most ${DEF_EQUIPMENT_3PLUS1_MAX_DISTINCT_CONSTRAINT_QUERIES} distinct normalized queries in total.`,
    })
  }
})
...
export const operator_config_patch = {
  ...
  args: {
    ...
    gearSetName: tool.schema.string().optional().describe('Exact set name paired with gearSetId. It only validates that same stable id; name-only set writes return 409.'),
    gearSetId: tool.schema.string().optional().describe('Stable set id from the active equipment catalog. A set write requires this id together with fillSlots:true.'),
    equipmentName: tool.schema.string().optional().describe('Exact equipment name paired with equipmentId. It only validates that same stable id; name-only piece writes return 409.'),
    equipmentId: tool.schema.string().optional().describe('Stable single-piece id from the active equipment catalog. It is required for every single equipment write.'),
    slotKey: tool.schema.enum(['armor', 'accessory2', 'accessory1', 'glove']).optional().describe('Slot for one equipment piece.'),
    fillSlots: tool.schema.boolean().optional().describe('Use true with gearSetId to fill all four slots. A set id without this explicit intent returns 409.'),
    ...
    equipments: tool.schema.array(tool.schema.object({
      equipmentId: tool.schema.string().min(1).max(180).describe('Exact stable equipment id returned by the equipment resolver. A preview blocks if the Operator Configuration library cannot resolve the same id.'),
      equipmentName: tool.schema.string().min(1).max(180).optional().describe('Exact catalog name paired with equipmentId; it validates that same stable id and is canonicalized before renderer enqueue.'),
