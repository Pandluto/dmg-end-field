...
For an explicit `3+1` request, materialize one equipment artifact after the profile is authorized. If the user already fixed one exact target set, call `def_data_equipment_3plus1_facts` for it. If no set was fixed, call `def_data_equipment_set_fit_shortlist` first: it reviews the complete catalog, requires a typed three-piece effect and a legal minimum-three-slot topology, and ranks set-effect fit before piece coverage. Choose only a returned set, then call its exact facts. Finally call `def_data_equipment_3plus1_plan` with the unchanged authorized profile/capability pair and the same artifact. `3+1` means at least three target-set memberships across 护甲、护手、配件1、配件2, not three unique items and not a mandatory off-set. A four-piece target-set combination is legal when it remains the best attribute/effect match; use an off-set piece only when it strictly improves the verified profile match. Treat physical-slot topologies as an internal solver constraint, not response content. Score pieces by how many verified ordered effect keys they match; prefer three matches, then two. If the typed duplicate policy allows it, a high-matching target-set accessory may occupy both accessory slots. If the typed facts still require profile evidence, obtain only the authorized missing profile before planning. A typed shortlist/facts/plan failure is terminal for this turn. Never rank directly from topology facts, fall back to legacy search, or invent a dropped main stat, elemental trigger, or damage benefit.
...
The native `skill` tool already returns this complete Skill. Never inspect the runtime Skill directory with `read`, `glob`, or `grep`. Generic file tools are only for the current session's `node/working/*.json`, plus an explicitly projected catalog-inspection flow under the exact `retrieval/<artifactId>` returned by `def_data_native_catalog_materialize`; typed planning itself consumes the opaque `artifactId` and never requires that file access. After one outside-session permission denial, do not try another path or file tool for that resource. A retrieval artifact is immutable evidence, never a node input or generated payload.
