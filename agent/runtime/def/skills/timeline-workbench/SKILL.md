...
This rule does **not** apply to a read-only request about “当前四人 / 全队 / 他们 / 每个人” and their weapons or equipment. Do not call `def_workbench_context` for those requests: use `def_data_team_loadouts` once for current configuration only if that identity is needed. For equipment/weapon library facts, sets, attributes, comparisons, or a provisional recommendation, call `def_data_native_catalog_materialize` once per domain/query, native-read its manifest, then use native `read`/`grep` only under the returned `retrieval/<artifactId>` root. This is an enforced route: do not call legacy `def_data_equipment`, `def_data_weapon`, `def_data_loadout_candidates`, game knowledge, operator catalog, Workbench/node tools, mutation, or approval in that turn. Do not edit files, request permission, or use a Work Node in that turn. For an explicit `3+1` request, after reading the named set manifest and full artifact, call `def_data_equipment_3plus1_facts` with its `artifactId` and exact set name. It returns slot topology plus stable id, fixedStat and effects for every target-set piece and off-set candidate. If it says `REQUIRES_ATTRIBUTE_PREFERENCE`, do not choose or rank the +1: ask only for the desired fixed stat and ordered secondary-effect type keys, or state that the priority cannot be verified. Never invent a dropped main stat, elemental trigger, or damage benefit. A named-guide/author request is a hard override, including “按他那篇攻略…先让我确认”: use `def_data_team_loadouts` only if current-team identity is needed, then `def_data_game_knowledge` once and `def_data_game_knowledge_section` once for its exact equipment/养成 section. Stop data-tool use and return a source-faithful proposal; do not resolve guide names against equipment/weapon catalogs, create a Work Node, or prepare an application.
