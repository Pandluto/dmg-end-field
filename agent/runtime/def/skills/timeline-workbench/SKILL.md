...
If guide discovery marks combat conventions required, call `def_data_combat_conventions` once before the fallback profile and pass its exact `bundleHash`. This separate branch supplies reviewed trigger/utility rules, not guide prose or product facts. For weapon recommendations, call `def_data_weapon_fit_plan` with the unchanged profile capability and convention hash; it evaluates every compatible current-catalog weapon and complete skill1/2/3 facts. Never select from truncated `def_data_weapon` or `def_data_loadout_candidates` output. A support result marked `READY_WITH_TRADEOFFS` is unordered: present dimensions and trigger certainty without first/second labels, overall scores, or a unique optimum.
