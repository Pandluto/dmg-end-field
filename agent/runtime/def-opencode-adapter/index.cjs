...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- The live Workbench snapshot skillCatalog is a trusted selected-operator catalog. If it contains an exact characterId + skillType + skillDisplayName match, use its skillId directly as runtimeSkillId and do not call def_data_skill for that exact skill. The semantic-term resolution rule above is the exception: do not derive a named action only from its A/B/E/Q letter. customHits, icon URLs, runtime snapshots, and damage multipliers are optional button metadata; never block or loop while trying to obtain them. def_data_skill returning no candidate does not invalidate an exact local skillCatalog entry.',
      ...
      '- NATIVE CATALOG ROUTE: For a read-only equipment/weapon filtering, comparison, attribute, item, or set question, call def_data_native_catalog_materialize exactly once for each requested domain and query. First native-read its returned manifestPath; then use native grep/read repeatedly only inside its returned root. Base the answer on actual artifact fields. entity-full means one exact set/item, substring-minimal means only matching fields, and domain-full-fallback means no deterministic match rather than absence from the game. Never materialize the same domain/query twice in one turn, never read raw local storage, and never turn this read-only evidence into an apply. This turn has an enforced tool boundary: do not call legacy equipment/weapon/loadout-candidates, game-knowledge, operator catalog, Workbench-context, node, mutation, approval, or any unrelated def tool after the catalog route starts.',
      '- EXPLICIT 3+1: When the user explicitly asks for “3+1”, materialize the named equipment set, native-read its manifest, inspect the exact full set artifact, then call def_data_equipment_3plus1_facts with that artifactId and exact set name. It returns stable ids, slots, fixedStat, effects, three-piece count, topology-only reasons, and full off-set accessory facts. If its state is REQUIRES_ATTRIBUTE_PREFERENCE, do not select, rank, or claim an optimal fourth item: ask one minimal question for the desired fixed stat plus ordered secondary-effect type keys, or report that those priorities are not evidenced. Never invent drop main stats, elemental triggers, or damage gains from a set name.',
      ...
      '- For any other read-only current-team weapons/equipment recommendation or “先让我确认” request, first call def_data_team_loadouts once only when current team identity is actually needed. Then use the NATIVE CATALOG ROUTE once per relevant domain and make a provisional recommendation only from those artifact fields plus the current-team facts. Do not call def_data_loadout_candidates, query one operator at a time, search a weapon by an operator name, materialize/fork/edit/use a Work Node, or request permission. A named guide remains the HARD GUIDE ROUTE and never enters this route.',
...
function getNativeHarnessSystem(binding, userText = '') {
  ...
  const turnInstruction = turnRoute.task === 'native-catalog'
    ? [
      'CURRENT TURN — EXECUTABLE READ-ONLY CATALOG CONTRACT:',
      'This is not a configuration request. Do not call operator, legacy equipment/weapon, loadout candidates, game knowledge, Workbench context, node, mutation, approval, or any unrelated DEF tool.',
      'For the named set, your first and only DEF data call must be def_data_native_catalog_materialize with domain="equipment" and the exact set query. Then native-read its returned manifestPath, native-read or grep only within that returned artifact root, and for an explicit 3+1 request call def_data_equipment_3plus1_facts with its artifactId and exact set name.',
      'Do not materialize a weapon: the user asked only for equipment. Do not treat “主副属性都对” as an evidenced priority. If facts return REQUIRES_ATTRIBUTE_PREFERENCE, state the supplied fixedStat/effects exactly and ask only for the desired fixed stat plus ordered secondary-effect type keys. Do not rank, apply, or invent a drop stat, elemental trigger, or benefit.',
    ].join('\n')
    : '';
  return {
    system: [defHarness.composeHarnessSystem(loaded.binding, loaded.resolved.artifactView), turnInstruction].filter(Boolean).join('\n\n'),
