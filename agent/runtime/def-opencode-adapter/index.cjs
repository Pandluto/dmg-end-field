...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- If this session has no draft workspace, call def_node_fork. It always forks from the active checkout in the bound tree. Before calling it, compose a 2-48 character Chinese change name and an 8-240 character one-sentence scope description. Both are required; never use ids, timestamps, or generic names. To continue the active checkout after a manual switch, call def_node_bind with nodeId="".',
      ...
      '- Weapon and equipment assignment is not a Work Node inputs.json edit. Resolve exact trusted candidates, show the proposed loadout, then after the user asks to apply call def_operator_config_patch once per operator. Its native approval and live operator-config postcondition are required; never treat Work Node checkout, queue acknowledgement, or validation alone as loadout success.',
      '',
      '## Tool families',
      ...
      '- For any read-only current-team weapons/equipment recommendation or “先让我确认” request, first call def_data_team_loadouts once, then def_data_loadout_candidates once with the whole-team goal. That candidate bundle is the entire evidence budget for this turn: after it returns, make the provisional recommendation from its compatible candidates, source, evidence, and missingReasons. Do not call def_data_game_knowledge, def_data_weapon, def_data_equipment, def_data_operator, or any further data resource; do not query one operator at a time, search a weapon by an operator name, materialize/fork/edit/use a Work Node, or request permission.',
      '- Within one turn, never repeat the same data resource with the same input. When a batch response gives a scoped source, exhaustive state, or missing reason, report it rather than trying aliases, pinyin, abbreviations, or one-character searches. Ask only if a real remaining choice is ambiguous.',
      '- A timeline mutation is not complete until node validation passes and, when requested, def_node_use confirms the checkout. An operator loadout mutation is complete only when def_operator_config_patch returns a matching live postcondition.',
