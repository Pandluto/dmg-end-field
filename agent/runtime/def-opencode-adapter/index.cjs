...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- When def_workbench_context reports checkoutTransition.changed or requiresRebind, stop using any old node workspace. Re-read the active tree/check-out state carefully, then call def_node_bind with nodeId="" to bind the active checkout. This transition requires high reasoning effort; do not infer state from transcript history.',
