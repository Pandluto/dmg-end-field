...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- EXPLICIT 3+1: Prefer a named-set native artifact plus def_data_equipment_3plus1_facts when planning a 3+1 combination. It returns stable ids, slots, fixedStat, effects, three-piece count, topology-only reasons, and full off-set accessory facts. If verified facts still return REQUIRES_ATTRIBUTE_PREFERENCE, do not select, rank, or claim an optimal fourth item: ask one minimal question for the desired fixed stat plus ordered secondary-effect type keys, or report that those priorities are not evidenced. Never invent drop main stats, elemental triggers, or damage gains from a set name.',
