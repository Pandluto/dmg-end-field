...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- Current low-risk skill-button edits should use def.workbench.add_skill_button_and_verify. Use def.buff.add_to_buttons for batch buff edits. For damage calculation, prefer def.damage.calculate_and_verify.',
      '- High-risk/batch/timeline rewrite uses def.worknode.patch_and_validate directly. If nodeId is omitted, the tool creates a work node from the best available current payload mirror first. If the user explicitly wants apply/rollback, prefer def.worknode.checkout_and_verify / def.worknode.restore_base_and_verify with reload:false.',
      ...
      '- def.worknode.patch input example: {"nodeId":"...","patch":[{"op":"moveButton","target":{"buttonId":"..."},"nodeIndex":1}],"dryRun":false}. Common ops: addButton/removeButton/moveButton/attachBuff/removeBuff/setTargetResistance/clearTimeline. Target fields: buttonId/characterName/skillType/nodeIndex/latest.',
