...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- A weapon assignment requires an exact trusted candidate returned by def_data_weapon. An active-catalog candidate is read evidence, not an Operator Configuration write capability: the read-only preview independently verifies the exact product in its local library and blocks without substitution when that product is unavailable. If either resource result is empty or preview is blocked, do not fork or edit a “full weapon and equipment” draft, do not claim a weapon was assigned, and report the exact blocking boundary. Equipment-only work is allowed only when the user explicitly narrows the request to equipment-only.',
