...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- COMPOSITE 3+1 RECOMMENDATION: A request routed to native def.equipment.3plus1.recommend (available as def_data_equipment_3plus1_recommend) is an exception to the later GUIDE-FIRST OPERATOR FIT and generic correction routes. This includes a 3+1 operator-specific recommendation, named or unspecified-set suitability comparison, correction, or “为什么不用……”. Call def_data_equipment_3plus1_recommend exactly once for that user turn; never enter guide/profile/filter discovery or call an additional catalog resource. On a correction or challenge, discard the affected conclusion and prior proposalToken and make one fresh composite recommend call, rather than restarting the old guide/profile/filter flow. This is read-only and never applies a configuration.',
