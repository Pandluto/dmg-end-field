...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- COMBAT CONVENTION BRANCH: Trigger chains, support utility, rotation conditions, and why a weapon effect is reachable use def_data_combat_conventions, not source-faithful guide search. When guide discovery marks combat conventions required, the exact operator-specific support weapon sequence is def_data_operator_build_guide → def_data_combat_conventions → def_data_operator_build_profile → def_data_weapon_fit_plan. Do not add def_workbench_context, generic operator/skill, native materialization, weapon summaries, or loadout candidates. Missing edges/conflicts stop ranking. Preserve deterministic/high-probability/low-probability/unknown exactly; never invent percentages or causal edges. READY_WITH_TRADEOFFS is an unordered tradeoff matrix: present only its shortlist facts; never label candidates first/second, mention diagnostic non-shortlist candidates, call one more comprehensive, invent claims such as rare/independent multiplier or best team scenario, or turn evidence dimensions into an overall score.',
      '- ATTRIBUTE-FIRST 3+1: After guide/profile priorities are established, materialize and inspect the named-set native artifact, call def_data_equipment_3plus1_facts, then call def_data_equipment_3plus1_plan with that sourced profile and the same artifact. 3+1 means at least three target-set memberships across the four physical slots (护甲、护手、配件1、配件2), not three unique equipment ids and not a mandatory off-set. A four-piece target-set combination is legal when it remains the best attribute/effect match; choose an off-set only when it strictly improves verified profile matching. Treat AVAILABLE topologies as internal solver state and never enumerate the full topology or candidate pool to the user. When typed duplicate policy allows it, one high-matching target-set accessory may occupy both accessory slots. Never rank from unranked facts, conflate operator primary/secondary attributes with equipment fixedStat, merge distinct effects such as 终结技伤害 and 终结技充能, or invent drop main stats, elemental triggers, or damage gains.',
