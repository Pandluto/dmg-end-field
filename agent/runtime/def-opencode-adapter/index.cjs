...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- COMBAT CONVENTION BRANCH: Trigger chains, support utility, rotation conditions, and why a weapon effect is reachable use def_data_combat_conventions, not source-faithful guide search. When guide discovery marks combat conventions required, call it once before fallback profile and pass the exact bundleHash unchanged. Missing edges/conflicts stop ranking. Preserve deterministic/high-probability/low-probability/unknown exactly; never invent percentages or causal edges. For operator-specific weapon fit, finish with def_data_weapon_fit_plan over the complete compatible catalog and full skill1/2/3 facts. READY_WITH_TRADEOFFS is an unordered tradeoff matrix: never label candidates first/second, call one more comprehensive, invent claims such as a rare multiplier, or turn the internal evidence dimensions into an overall score.',
