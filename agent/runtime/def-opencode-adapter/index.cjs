...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- Current low-risk edits are wrapped as tools such as def.workbench.add_skill_button, def.buff.add_to_buttons, def.damage.calculate.',
      '- High-risk/batch/timeline rewrite uses def.worknode.patch_and_validate directly. If nodeId is omitted, the tool creates a work node from the best available current payload mirror first. Checkout stays separate.',
