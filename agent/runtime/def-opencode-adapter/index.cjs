...
function buildAgentPrompt(skillId) {
  ...
  if (skillId === 'workbench') {
    return [
      ...
      '- Weapon and equipment assignment is not a Work Node inputs.json edit. First call def_operator_config_preview for one exact proposed configuration and show only its verified result. That preview does not create a branch or apply anything. Only after the user’s later, explicit application instruction (for example “确认应用/换上/就按这套应用”) may you call def_operator_config_patch once with the unchanged proposalToken. A comparison, correction, question such as “为什么不用…”, or any changed slot/candidate/priority invalidates the old proposal: recompute a preview and wait for a new explicit application instruction. Supply a short Agent-written nodeTitle and a concise nodeDescription for the visible horizontal configuration branch; do not use a fixed operator-config title. Its native approval and live operator-config postcondition are required; never treat Work Node checkout, queue acknowledgement, or validation alone as loadout success.',
      '',
      '## Tool families',
      ...
      '- EXPLICIT 3+1: Prefer a named-set native artifact plus def_data_equipment_3plus1_facts when planning a 3+1 combination. 3+1 means exactly three target-set memberships across the four physical slots (护甲、护手、配件1、配件2), not three unique equipment ids. Enumerate the returned AVAILABLE topologies before comparing candidates: the off-set may be armor, glove, accessory1, or accessory2. When the typed duplicate policy allows it, one target-set accessory may occupy both accessory slots. Never reduce this to a hard-coded “护甲+护手+一个配件，再补散件配件” rule. If verified facts still return REQUIRES_ATTRIBUTE_PREFERENCE, do not select, rank, or claim an optimal item: ask one minimal question for the desired fixed stat plus ordered secondary-effect type keys, or report that those priorities are not evidenced. Never invent drop main stats, elemental triggers, or damage gains from a set name.',
      '- HARD GUIDE ROUTE: Call def_data_game_knowledge only when the current user explicitly names a community guide, author, link, quoted guide wording, or asks what a guide says (for example YZ 的某篇攻略). Generic equipment/weapon/operator/set terms, a build request, or a comparison are not guide intent and must not trigger game-knowledge search. For an actual guide request, first call def_data_game_knowledge once with the user wording, then def_data_game_knowledge_section exactly once using that candidate’s exactReadPolicy.requiredSectionId (or recommendedSection.sectionId only if no policy exists). Do not call def_data_team_loadouts when the returned guide title itself identifies the four-person roster: the title is the sole roster source for this answer. Only if a returned guide does not identify its requested roster may team state be read once before its single section read. Then stop tool use and answer. A later user confirmation such as “那你配装吧” must call def_team_loadout_plan exactly once; it reuses this session’s exact guide section and never searches the guide or catalog again. If its state is REQUIRES_CONFIRMATION, show every returned decision and option exactly, then stop. After the user plainly chooses an offered option, call def_team_loadout_plan_revise with only that returned decisionId/optionId; if its new state is READY and the user asked to equip, call def_team_loadout_plan_apply exactly once. Never guess a decision id, substitute products, apply a non-READY plan, or call per-item resources. exactReadPolicy is a hard execution limit, not advice: never read a second section, including 阵容概述. “先让我确认” means a source-faithful draft only; it never authorizes mapping guide names to product-library ids or preparing an application. For a four-person build question, list the four names exactly as the matched title gives them, but never say or imply that they equal, match, differ from, or are equipped by the current team. State only verbatim-level facts from that one section; preserve its names, thresholds, and notation exactly (for example, never rename 专1+ as M1+). Mark anything absent as 待确认. End at the draft itself—do not promise, propose, or describe any later application, current-state comparison, catalog lookup, or next tool call. Never call def_data_loadout_candidates, def_data_weapon, def_data_equipment, any per-person resource, file tool, permission, or Work Node tool after this route starts.',
      '- For any other read-only current-team weapons/equipment recommendation or “先让我确认” request, first call def_data_team_loadouts once only when current team identity is actually needed. Choose the narrowest trusted resource that can support the question; upgrade to a native catalog artifact when a full-field comparison or evidence trail is needed. Make a provisional recommendation only from fields actually returned. Do not materialize/fork/edit/use a Work Node, or request permission. A named guide remains the HARD GUIDE ROUTE.',
