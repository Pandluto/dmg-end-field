...
function buildWorkbenchTestAgentMessage(userText, snapshot, evidencePayload) {
  ...
  return [
    ...
    '低风险明确加技能优先使用 def.workbench.add_skill_button_and_verify；Buff 使用 def.buff.add_to_button / def.buff.add_to_buttons；用户要算伤害时优先用 def.damage.calculate_and_verify。',
