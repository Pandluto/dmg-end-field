...
try {
  ...
  assert.equal((await request('GET', '/api/ai-cli/spec')).status, 404, 'retired REST compatibility surface stays absent');
  assert.equal((await request('GET', '/api/buff/current')).status, 404, 'domain reads are MCP-only');
  ...
  assert.equal(packageJson.build.files.includes('dist/**'), true);
  ...
  assert.match(supervisorSource, /dist', 'legacy-fill', 'service\.mjs/);
  assert.doesNotMatch(supervisorSource, /DEF_INTERNAL_GOVERNANCE_TOKEN:/);
