...
async function start(text, sessionId = '') {
  ...
  return request(path, {
    method: 'POST', headers,
    body: JSON.stringify({ protocolVersion: 1, rawUserText: text, clientTurnId: clientTurnId(), ingressMode: 'pure-blackbox' }),
  });
}
...
try {
  if (command === 'status') output(await request('/def-agent/interop/v1/status'));
  else if (command === 'state') output(await request('/def-agent/interop/v1/state'));
    ...
    output(await request(`/def-agent/interop/v1/sessions/${encodeURIComponent(arguments_[0])}/transcript`));
  } else {
    fail('Unknown command.', { command, usage: 'status | state | hello | start <text> | continue <sessionId> <text> | stop <sessionId> <turnId> | transcript <sessionId>' });
  }
} catch (error) {
