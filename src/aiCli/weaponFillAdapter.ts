...
export const weaponFillAdapter: AgentFillDomainAdapter<WeaponDraft> = {
  ...
  buildTaskPackage() {
    ...
    return { lines: [`[info] weapon.fill.task ready: name=${draft.name} skills=${Object.keys(draft.skills).length}`], data: {
      ...
      approvalSaveWarning: 'IMPORTANT: After REST weapon.fill.apply, review the proposal in the supported MCP Fill page. The retired /ai-cli page is unavailable. Do not submit another weapon.fill.apply while a pending proposal exists. For stale backlog, call proposal.clear through REST, then resubmit only the current proposal.',
    } };
  },
