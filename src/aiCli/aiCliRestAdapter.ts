  ...
  if (request.method === 'GET' && request.path === '/api/agent/guide') {
    return jsonResponse(200, {
      ...
      mainWorkbenchControl: {
        purpose: 'Read-only renderer bridge diagnostics plus canonical DEF tool discovery. Model-side mutation must use DEF tool routes.',
        storage: {
        ...
        rules: [
          ...
          'Renderer commands are internal transport. Direct command enqueue is denied because it bypasses registry metadata, current gate, and approval policy.',
          ...
          'Read the current snapshot only after a canonical typed tool reports command verification.',
