...
export function validateLegacyFillDraft(domain: string, input: unknown) {
  ...
  if (domain === 'operator') {
    const parsed = parseOperatorFillJsonPayload(typeof input === 'string' ? input : JSON.stringify(input));
    if (!parsed.value) return { valid: false, errors: parsed.errors, warnings: [] };
    const result = operatorFillDomainCore.validate(parsed.value);
    return result.ok ? { valid: true, errors: [], warnings: [], normalized: result.normalized } : { valid: false, errors: result.errors, warnings: [] };
  }
  if (domain === 'equipment') {
    const parsed = parseEquipmentFillJsonPayload(typeof input === 'string' ? input : JSON.stringify(input));
    if (!parsed.value) return { valid: false, errors: parsed.errors, warnings: [] };
    const result = equipmentFillDomainCore.validate(parsed.value);
    return result.ok ? { valid: true, errors: [], warnings: [], normalized: result.normalized } : { valid: false, errors: result.errors, warnings: [] };
  }
...
export function getLegacyFillTemplate(domain: string) {
  ...
  if (domain === 'operator') return {
    format: 'operator.fill',
    schema: domainCores.operator.schema(),
    data: { tool: 'operator.fill', schema: createOperatorFillDraftSchema() },
  };
  if (domain === 'equipment') return {
    format: 'equipment.fill',
    schema: domainCores.equipment.schema(),
    data: { tool: 'equipment.fill', schema: createEquipmentFillDraftSchema() },
  };
