...
export const BUFF_TYPE_CATALOG: readonly BuffTypeCatalogEntry[] = [
  ...
  createEntry({
    ...
    valueStyle: 'ratio',
    aliases: ['物理脆弱', '物脆弱'],
    positivePatterns: ['物理脆弱', '物理抗性降低'],
    negativePatterns: ['物理伤害提高', '物理增幅', '受到的物理伤害提高', '物理易伤'],
    examplePhrases: ['目标陷入物理脆弱15%'],
    notes: '偏向脆弱描述。',
    canInferFromImplicitText: false,
  }),
  createEntry({
  ...
  createEntry({
    ...
    examplePhrases: ['全属性抗性降低10点', '腐蚀降低的最大抗性提升至原本的1.1倍'],
    notes: '抗性区点数，数值按点录入。腐蚀异常快照使用该类型。若文本说腐蚀降低的最大抗性提升至原本的若干倍，使用 multiplier.coefficient 记录直接系数；“额外提升原本的20%”记为 1.2。腐蚀持续时间变化不进入当前计算。',
    canInferFromImplicitText: true,
  }),
  createEntry({
...
export function buildBuffTypeCatalogPromptSection() {
  const header = [
    ...
    '5. 腐蚀文本中的持续时间变化同样舍弃；若同一句还写有“降低的最大抗性提升至原本的1.1倍”或“额外提升原本的20%”，保留后半句并输出 type=allCorrosion、category=condition、multiplier.coefficient=1.1 或 1.2，不写普通 value。',
    '6. 只有 effectKind=extraHit 时，type 才允许为空字符串。',
