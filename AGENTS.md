...
- 开发者 Skill 路由集中维护在 `.agents/skill-routing.md`；用户说“harness 审计辅助”或提供 DEF 会话 ID 要求导出、审计并生成返修提示词时，必须读取并执行 `.agents/skills/harness-audit-assistant/SKILL.md`。`.agents/skills/**` 仅供开发 Codex 使用，严禁与 `agent/runtime/def/skills/**` 的 def-opencode 产品运行时 Skills 混放或互相引用。
